...
import dev.slne.surf.surfapi.gradle.extensions.SurfApiExtension.Companion.surfApi
...
class SurfApiPlugin : Plugin<Project> {
    override fun apply(target: Project) = target.run {
        ...
        afterEvaluate {
            val platformProperty = surfApi.platform
            require(platformProperty.isPresent) { "No platform specified for Surf API" }

            val configuration = surfApi.configuration!!
            val platform = platformProperty.get()
            ...
            when (configuration) {
                is Paper -> {
                    applyPaperConfiguration(configuration)
                }
