...
//        addDependency("com.github.megavexnetwork.scoreboard-library", "scoreboard-library-modern", SCOREBOARD_LIBRARY_VERSION);
    ...
    private void addDependency(@NotNull String group, @NotNull String artifact,
        ...
        resolver.addDependency(
            new Dependency(new DefaultArtifact("%s:%s:%s".formatted(group, artifact, version)),
                null));
    }
    ...
    private PluginLibraries loadFromFile() {
        try (final InputStream inputStream = getClass().getResourceAsStream(
            ...
            if (inputStream != null) {
                ...
                logger.warn("Failed to load paper-libraries.json");
    ...
    private record PluginLibraries(Map<String, String> repositories, List<String> dependencies) {
        ...
        private Stream<RemoteRepository> asRepositories() {
            return repositories.entrySet().stream()
                .filter(
                    entry -> !MavenLibraryResolverReflection.isMavenCentralUrl(entry.getValue()))
        ...
        static {
            try {
                ...
                throw new RuntimeException(
                    "Failed to access MavenLibraryResolver's mavenCentralUrls", e);
            }
        }
