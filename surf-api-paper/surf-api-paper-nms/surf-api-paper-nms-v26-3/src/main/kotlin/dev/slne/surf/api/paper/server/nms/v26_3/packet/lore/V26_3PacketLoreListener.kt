...
@Suppress("ClassName")
object V26_3PacketLoreListener : PacketListener {
    ...
    fun onClientboundPlayerChatPacket(event: ClientboundPlayerChatPacket): ClientboundPlayerChatPacket {
        if (!hasAnyHandlers()) return event
        val unsignedContent = event.unsignedContent().orElse(null) ?: return event
    ...
    private fun makeUpdatedItemStack(
        ...
        val matchingKeyedHandlers = if (keyedSnapshot.isNotEmpty()) {
            val originalBukkitStack = CraftItemStack.asBukkitMirror(original)
        ...
        */
        ...
        val bukkitStack = CraftItemStack.asBukkitMirror(item)
