...
class V26_2SurfPaperNmsPlayerBridgeImpl : SurfPaperNmsPlayerBridge {
    ...
    override fun <T> withMessageSignatureCacheLock(player: Player, block: () -> T): T? {
        val connection = player.toNms().connection ?: return null
        val cache = V26_2NmsReflections.getMessageSignatureCache(connection)
    ...
    override fun createChatSessionSnapshot(player: Player): PlayerChatSessionSnapshot? {
        ...
        return synchronized(lastSeenMessages) {
            synchronized(messageSignatureCache) {
                val nextChatIndex = V26_2NmsReflections.getNextChatIndex(connection)
                val chatSession = V26_2NmsReflections.getRemoteChatSession(connection) ?: return null

                val lastSeenMessagesMirror = LastSeenMessagesValidatorMirror(
                    lastSeenCount = V26_2NmsReflections.getLastSeenCountFromMessageValidator(lastSeenMessages),
                    trackedMessages = V26_2NmsReflections.getTrackedMessagesFromMessageValidator(lastSeenMessages)
                        .map { entry ->
                            entry?.let {
                    ...
                    lastPendingMessage = V26_2NmsReflections.getLastPendingMessageFromMessageValidator(lastSeenMessages)
                        ?.bytes()
                )

                val messageSignatureCacheEntries =
                    V26_2NmsReflections.getEntriesFromMessageSignatureCache(messageSignatureCache)
                ...
                val messageDecoder = V26_2NmsReflections.getSignedMessageDecoder(connection)
                val signedMessageChain = messageDecoder.signedMessageChain()

                val chatChainMirror = IncomingChatChainMirror(
                    nextLinkIndex = V26_2NmsReflections.getNextLinkFromSignedMessageChain(signedMessageChain)?.index,
                    lastTimeStamp = V26_2NmsReflections.getLastTimeStampFromSignedMessageChain(signedMessageChain)
                )
    ...
    override fun applyChatSessionSnapshot(player: Player, snapshot: PlayerChatSessionSnapshot) {
        ...
        synchronized(lastSeenMessages) {
            synchronized(messageSignatureCache) {
                V26_2NmsReflections.setNextChatIndex(connection, snapshot.nextChatIndex)

                val lastSeenCount = V26_2NmsReflections.getLastSeenCountFromMessageValidator(lastSeenMessages)
                val trackedMessages = V26_2NmsReflections.getTrackedMessagesFromMessageValidator(lastSeenMessages)
                ...
                V26_2NmsReflections.setLastPendingMessageFromMessageValidator(lastSeenMessages, lastPendingMessage)
        ...
        V26_2NmsReflections.setRemoteChatSession(connection, chatSession)
        V26_2NmsReflections.setHasLoggedExpiry(connection, false)

        val messageChain = SignedMessageChain(nmsPlayer.uuid, chatSession.sessionId)
        V26_2NmsReflections.setLastTimeStampFromSignedMessageChain(
            messageChain,
            snapshot.incomingChatChain.lastTimeStamp
        )
        V26_2NmsReflections.setNextLinkFromSignedMessageChain(
            messageChain,
        ...
        V26_2NmsReflections.setSignedMessageDecoder(connection, signedMessageDecoder)

        val chatChain = V26_2NmsReflections.getChatMessageChain(connection)
        chatChain.append {
