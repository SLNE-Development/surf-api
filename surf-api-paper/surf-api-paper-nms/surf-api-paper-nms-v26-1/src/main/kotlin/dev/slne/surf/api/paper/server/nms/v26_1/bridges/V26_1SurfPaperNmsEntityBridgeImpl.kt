...
class V26_1SurfPaperNmsEntityBridgeImpl : SurfPaperNmsEntityBridge {
    ...
    override fun captureVehicleNbt(rootVehicle: Entity): ByteArray {
        ...
        return ProblemReporter.ScopedCollector(VEHICLE_LOGGER).use { reporter ->
            ...
            serializeTagToBytes(output.buildResult())
        }
    }
    ...
    override fun restoreVehicle(
        ...
        var tag = deserializeTagFromBytes(nbt)
    ...
    private fun deserializeTagFromBytes(data: ByteArray): CompoundTag {
        try {
            return NbtIo.readCompressed(ByteArrayInputStream(data), NbtAccounter.unlimitedHeap())
        } catch (ex: IOException) {
            throw RuntimeException(ex)
        }
    }

    private fun serializeTagToBytes(compound: CompoundTag): ByteArray {
        try {
    ...
    companion object {
        private val VEHICLE_LOGGER = ComponentLogger.logger("V26_1SurfPaperNmsEntityBridgeImpl Vehicle")
    }
}
