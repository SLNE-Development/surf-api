...
import dev.slne.surf.api.paper.builder.ItemDsl
...
class ViewButtonStateScope<T> internal constructor(val value: T) {
     ...
     * @param init customization block applied to the freshly built [ItemStack]
     */
    fun item(
        material: Material,
        amount: Int = 1,
    ...
    /**
     * Renders this state as [type].
     *
     * @param type the [ItemType] shown while the button is in this state
     * @param amount the stack size; defaults to `1`
     * @param init customization block applied to the freshly built [ItemStack]
     * @see item
     */
    fun item(
        ...
        itemFactory = { buildItem(type, amount, init) }
    }
 ...
 *
 * @param T the type identifying the button's states
 * @see dev.slne.surf.api.paper.inventory.framework.view.button.statefulButton
 */
@InventoryFrameworkDSL
 ...
 * ```
 *
 * @see dev.slne.surf.api.paper.inventory.framework.view.button.toggleButton
 */
@InventoryFrameworkDSL
