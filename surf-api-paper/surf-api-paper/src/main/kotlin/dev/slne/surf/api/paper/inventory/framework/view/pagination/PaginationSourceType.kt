...
internal sealed interface PaginationSourceType<T> {
     ...
     * Re-evaluated on every page load.
     */
    class ComputedAsync<T>(val provider: (Context) -> CompletableFuture<Iterable<T>>) :
        PaginationSourceType<T>
     ...
     */
    class LazyAsync<T>(val provider: (Context) -> CompletableFuture<Iterable<T>>) :
        PaginationSourceType<T>
}
