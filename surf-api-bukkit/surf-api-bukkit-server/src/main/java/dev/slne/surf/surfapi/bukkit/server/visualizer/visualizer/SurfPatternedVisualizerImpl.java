...
public class SurfPatternedVisualizerImpl extends SurfVisualizerImpl implements SurfPatternedVisualizer {
    ...
    private void restartVisualizing() {
        ...
        calculateLocationsFromPoints().forEach(location -> addVisualLocation(location, visualMaterial, visualMaterialConsumer));
    }
    ...
    private List<Location> calculateLocationsFromPoints() {
        ...
        for (Location visualPoint : visualPoints) {
            if (renderAtHighestPoint) {
                locations.add(visualPoint.getWorld().getHighestBlockAt(visualPoint, HeightMap.MOTION_BLOCKING_NO_LEAVES).getLocation().clone().add(0, 1, 0));
            } else {
    ...
    /**
     * Walks from point A to point B in a straight line, generating a list of locations along the way.
     *
     ...
     */
    private @NotNull List<Location> walkPointAToB(@NotNull Location currentPoint, @NotNull Location nextPoint) { // TODO: is the y coordinate right here?
