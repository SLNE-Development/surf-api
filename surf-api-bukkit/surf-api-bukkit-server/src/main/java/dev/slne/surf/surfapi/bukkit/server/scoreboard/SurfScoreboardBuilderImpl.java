...
public class SurfScoreboardBuilderImpl implements SurfScoreboardBuilder {
  ...
  @Contract("_, _, _ -> new")
  private static @NotNull SidebarAnimation<Component> createGradientAnimation(Component component,
      String firstHex, String secondHex) {
    ...
    while (phase < 1) {
      frames.add(
          MiniMessage.miniMessage().deserialize(
              "<gradient:%s:%s:%s><text></gradient>".formatted(firstHex, secondHex, phase),
              textPlaceholder)
      );
      phase += step;
    }
  ...
  @Override
  public SurfScoreboardBuilder addAnimatedLine(
      @NotNull SidebarAnimation<@NotNull SidebarComponent> animation) {
  ...
  public SurfScoreboardBuilder addAnimatedLine(@NotNull List<@NotNull Component> frames) {
    ...
    CollectionSidebarAnimation<@NotNull Component> animation = new CollectionSidebarAnimation<>(
        frames);
  ...
  @Override
  public SurfScoreboardBuilder addGradientLine(@NotNull Component text, @NotNull TextColor start,
      @NotNull TextColor end) {
    checkNotNull(text, "text");
    checkNotNull(start, "start");
    checkNotNull(end, "end");

    SidebarAnimation<Component> gradient = createGradientAnimation(text, start.asHexString(),
        end.asHexString());
  ...
  public @NotNull SurfAutoUpdatableScoreboard buildAutoUpdatable() {
    return new SurfAutoUpdatableScoreboardImpl(title, maxLines, sidebarComponentBuilder.build(),
        animations);
  }

  @Override
  public @NotNull SurfAutoUpdatablePlayerScoreboard buildAutoUpdatablePlayer() {
    return new SurfAutoUpdatablePlayerScoreboardImpl(title, maxLines,
        sidebarComponentBuilder.build(), animations);
  }
}
