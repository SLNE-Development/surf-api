...
/**
 * PacketLoreListener is a class that implements PacketListenerAbstract and is responsible for handling the modification
 * of lore on item stacks in packet events.
 */
@ApiStatus.Internal
public final class PacketLoreListener extends PacketListenerAbstract {
    /**
     * Represents an instance of the PacketLoreListener class.
     * This variable is used to access the PacketLoreListener class functionality.
     * It is a static final variable, meaning it cannot be changed or reassigned.
     * This variable is initialized with a new instance of the PacketLoreListener class.
     */
     ...
     * <p>
     * The lore prefix is a {@link Component} object with the value "§q".
     * It is used in conjunction with other {@link SurfBukkitPacketLoreHandler} instances to modify the lore of an item stack.
     *
    ...
    /**
     * This method is called when a packet is received. It checks if the packet type is CREATIVE_INVENTORY_ACTION and then
     * retrieves the item stack from the packet. The received item stack is then passed to the getCleanItemStack() method,
     * which removes any lore that starts with the specified prefix. The modified item stack is then set back to the packet.
     *
     ...
     * This method handles the modification of the packet before sending it.
     *
    ...
    private ItemStack getUpdatedItemStack(@NotNull ItemStack item) {
        final org.bukkit.inventory.ItemStack bukkitStack = SpigotConversionUtil.toBukkitItemStack(item).clone();

        bukkitStack.editMeta(meta -> {
            final PersistentDataContainer pdc = meta.getPersistentDataContainer();
            final List<Component> lore = Optional.ofNullable(meta.lore()).map(ArrayList::new).orElse(new ArrayList<>());
    ...
    /**
     * Returns a clean version of the given ItemStack by removing specific lore components.
     * If the input stack is null, null is returned.
     *
