package dev.slne.surf.surfapi.core.api.config.serializer.collections;
...
public class MapSerializer implements TypeSerializer<Map<?, ?>> {
    ...
    if (node.isMap()) {
      ...
      if (valueSerializer == null) {
        throw new SerializationException(type,
            "No type serializer available for value type " + value);
      }
      ...
      for (Map.Entry<Object, ? extends ConfigurationNode> ent : node.childrenMap().entrySet()) {
        final @Nullable Object deserializedKey = deserialize(key, keySerializer, "key",
            keyNode.set(ent.getKey()), node.path());
        final @Nullable Object deserializedValue = deserialize(value, valueSerializer, "value",
            ent.getValue(), ent.getValue().path());
        if (deserializedKey == null || deserializedValue == null) {
          continue;
        }
        if (keySerializer instanceof WriteBack) {
          if (serialize(key, keySerializer, deserializedKey, "key", keyNode, node.path())
              && !ent.getKey().equals(requireNonNull(keyNode.raw(), "Key must not be null!"))) {
  ...
  private @Nullable Object deserialize(Type type, TypeSerializer<?> serializer, String mapPart,
      ConfigurationNode node, NodePath path) {
    try {
      return serializer.deserialize(type, node);
    } catch (SerializationException ex) {
      ex.initPath(node::path);
      ComponentLogger.logger()
          .error("Could not deserialize {} {} into {} at {}", mapPart, node.raw(), type, path);
    }
    return null;
  }

  @Override
  public void serialize(Type type, @Nullable Map<?, ?> obj, ConfigurationNode node)
  ...
  private boolean serialize(Type type, TypeSerializer serializer, Object object, String mapPart,
      ConfigurationNode node, NodePath path) {
    try {
      ...
      ex.initPath(node::path);
      ComponentLogger.logger()
          .error("Could not serialize {} {} from {} at {}", mapPart, object, type, path);
    }
    return false;
  }
