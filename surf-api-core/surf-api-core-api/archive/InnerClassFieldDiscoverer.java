...
public final class InnerClassFieldDiscoverer implements FieldDiscoverer<Map<Field, Object>> {
  ...
  private final FieldDiscoverer<Map<Field, Object>> delegate = (FieldDiscoverer<Map<Field, Object>>) FieldDiscoverer.object(
      target -> {
        ...
        if (ConfigurationPart.class.isAssignableFrom(type) && !this.instanceMap.containsKey(type)) {
          try {
            ...
            if (type.getEnclosingClass() != null && !Modifier.isStatic(type.getModifiers())) {
              ...
              if (instance == null) {
                throw new SerializationException(
                    "Cannot create a new instance of an inner class " + type.getName()
                        + " without an instance of its enclosing class " + type.getEnclosingClass()
                        .getName());
              }
            ...
            throw new SerializationException(ConfigurationPart.class,
                target + " must be a valid ConfigurationPart", e);
          }
        } else {
  ...
  @Override
  public @Nullable <V> InstanceFactory<Map<Field, Object>> discover(AnnotatedType target,
      FieldCollector<Map<Field, Object>, V> collector) throws SerializationException {
    final Class<?> clazz = erase(target.getType());
    if (ConfigurationPart.class.isAssignableFrom(clazz)) {
      final FieldDiscoverer.@Nullable InstanceFactory<Map<Field, Object>> instanceFactoryDelegate = this.delegate.<V>discover(
          target, (name, type, annotations, deserializer, serializer) -> {
            if (!erase(type.getType()).equals(
                clazz.getEnclosingClass())) { // don't collect synth fields for inner classes
      ...
      if (instanceFactoryDelegate instanceof FieldDiscoverer.MutableInstanceFactory<Map<Field, Object>> mutableInstanceFactoryDelegate) {
        return new MutableInstanceFactory<>() {
          ...
          @SuppressWarnings("unchecked")
          @Override
          public void complete(Object instance, Map<Field, Object> intermediate)
            ...
            try {
              while (iter.hasNext()) { // manually merge any mergeable maps
                ...
                if (entry.getKey().isAnnotationPresent(MergeMap.class)
                    && Map.class.isAssignableFrom(entry.getKey().getType()) && intermediate.get(
                    entry.getKey()) instanceof Map<?, ?> map) {
                  iter.remove();
                  @Nullable Map<Object, Object> existingMap = (Map<Object, Object>) entry.getKey()
                      .get(instance);
                  if (existingMap != null) {
          ...
          public Object complete(Map<Field, Object> intermediate) throws SerializationException {
            @Nullable Object targetInstance = InnerClassFieldDiscoverer.this.instanceMap.get(
                GenericTypeReflector.erase(target.getType()));
