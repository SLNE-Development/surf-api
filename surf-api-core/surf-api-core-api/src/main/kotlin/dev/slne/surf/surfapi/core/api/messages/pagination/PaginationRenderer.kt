...
interface SuspendPaginationRenderer {
    ...
    suspend fun CoroutineScope.renderFooter(
        ...
        if (page == 1 && pages == 1) {
            ...
            val nav = buildText {
                val first = async { renderPreviousPageButton(firstPage, changePageEvent(1), page > 1) }
                val prev = async { renderPreviousPageButton(previousPage, changePageEvent(page - 1), page > 1) }
                val next = async { renderNextPageButton(nextPage, changePageEvent(page + 1), page < pages) }
                val last = async { renderNextPageButton(lastPage, changePageEvent(pages), page < pages) }
