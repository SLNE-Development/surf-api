...
internal class RandomSelectorImpl<E>(
    ...
    init {
        ...
        val pmf = elements.map { org.apache.commons.math3.util.Pair.create(it.key, it.doubleValue) }
...
internal class FlowRandomSelectorImpl<E>(
    ...
    @Suppress("OVERRIDE_DEPRECATION")
    override fun flow(randomGenerator: RandomGenerator): Flow<E> = createFlow(randomGenerator.toApache())
    override fun flow(): Flow<E> = createFlow(randomGenerator)
    override fun flowOrNull(successRate: Double): Flow<E?> = throw UnsupportedOperationException("FlowRandomSelector does not support flowOrNull operation.")
