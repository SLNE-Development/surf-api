...
public interface Reference2IntMap<K> extends Reference2IntFunction<K>, Map<K, Integer> {
	...
	default void forEach(final java.util.function.BiConsumer<? super K, ? super Integer> consumer) {
		final ObjectSet<Reference2IntMap.Entry<K>> entrySet = reference2IntEntrySet();
		final Consumer<Reference2IntMap.Entry<K>> wrappingConsumer = (entry) -> consumer.accept((entry.getKey()), Integer.valueOf(entry.getIntValue()));
	 ...
	 * @since 8.0.0
	 */
	default int computeIfAbsent(final K key, final Reference2IntFunction<? super K> mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final int v = getInt(key), drv = defaultReturnValue();
		if (v != drv || containsKey(key)) return v;
