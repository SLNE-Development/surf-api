	...
	private final class EntrySet extends AbstractObjectSet<Reference2BooleanMap.Entry<K>> implements FastEntrySet<K> {
		...
		public ObjectIterator<Reference2BooleanMap.Entry<K>> iterator() {
			return new ObjectIterator<Reference2BooleanMap.Entry<K>>() {
				...
				@Override
				@SuppressWarnings("unchecked")
				public void forEachRemaining(final Consumer<? super Reference2BooleanMap.Entry<K>> action) {
					// Hoist containing class field ref into local
					final int max = size;
					while (next < max) {
						action.accept(new AbstractReference2BooleanMap.BasicEntry<>((K)key[curr = next], value[next++]));
					}
				}
			};
		}
		...
		final class EntrySetSpliterator extends ObjectSpliterators.EarlyBindingSizeIndexBasedSpliterator<Reference2BooleanMap.Entry<K>> implements ObjectSpliterator<Reference2BooleanMap.Entry<K>> {
			...
			protected final Reference2BooleanMap.Entry<K> get(int location) {
				return new AbstractReference2BooleanMap.BasicEntry<>((K)key[location], value[location]);
			}
		...
		@Override
		@SuppressWarnings("unchecked")
		public void forEach(final Consumer<? super Reference2BooleanMap.Entry<K>> action) {
			// Hoist containing class field ref into local
			for (int i = 0, max = size; i < max; ++i) {
				action.accept(new AbstractReference2BooleanMap.BasicEntry<>((K)key[i], value[i]));
			}
		}
		...
		@Override
		@SuppressWarnings("unchecked")
		public void fastForEach(final Consumer<? super Reference2BooleanMap.Entry<K>> action) {
			final BasicEntry<K> entry = new BasicEntry<>();
			// Hoist containing class field ref into local
			for (int i = 0, max = size; i < max; ++i) {
				entry.key = (K)key[i];
