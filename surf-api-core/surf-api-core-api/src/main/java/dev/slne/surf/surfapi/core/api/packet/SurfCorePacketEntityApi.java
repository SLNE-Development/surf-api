...
public interface SurfCorePacketEntityApi { // TODO: look at, equipment
  ...
  <T extends PacketEntity<T> & Spawnable> T spawnEntity(@NotNull Class<T> entityClass,
      @NotNull UUID uuid, @Nullable Consumer<T> initializer);

  default <T extends PacketEntity<T> & Spawnable> T spawnEntity(@NotNull Class<T> entityClass,
      @NotNull UUID uuid) {
   ...
   * @return An Optional containing the PacketEntity if an entity with the specified UUID exists,
   * otherwise empty.
   */
  <T extends PacketEntity<T>> Optional<T> getEntity(@NotNull UUID uuid);

  /**
   * Retrieves the PacketEntity with the specified UUID (same as {@link #getEntity(UUID)} but with a
   * class parameter wich can sometimes be useful).
   *
   * @param uuid        The UUID of the entity to retrieve.
   ...
   * @param <T>      the type of the entity's metadata
   * @return an Optional representing the SurfEntity with the specified entityId, or an empty
   * Optional if no entity is found
   */
  <T extends PacketEntity<T>> Optional<T> getEntity(
      @Range(from = 0L, to = Integer.MAX_VALUE) int entityId);

  /**
   * Retrieves the SurfEntity with the specified entityId (same as {@link #getEntity(int)} but with
   * a class parameter wich can sometimes be useful).
   *
   * @param entityId    the ID of the entity
   ...
   * <p>
   * This method will despawn the entity and remove it from the entity registry. Any calls to any
   * methods on the entity after this method will end up in undefined behavior.
   * </p>
   *
  ...
  PacketExperienceOrb spawnExperienceOrb(@NotNull Vector3i position, short experience,
  ...
  default void spawnExplosion(@NotNull Vector3d position,
      ...
      @Nullable Float explosionSoundRange,
      @ArrayLenRange(from = 1) UUID @NotNull ... viewers) {
    spawnExplosion(position, power, relativeAffectedBlocks, playerVelocity, null, null, blockAction,
        explosionSound, explosionSoundRange, viewers);
  }
  ...
  default void spawnExplosion(@NotNull Vector3d position,
      ...
      @Nullable Float explosionSoundRange,
      @ArrayLenRange(from = 1) UUID @NotNull ... viewers) {
    spawnExplosion(position, power, relativeAffectedBlocks, playerVelocity, null, null, null,
        explosionSound, explosionSoundRange, viewers);
  }
  ...
  default void spawnExplosion(@NotNull Vector3d position,
      ...
      @Nullable Vector3f playerVelocity,
      @ArrayLenRange(from = 1) UUID @NotNull ... viewers) {
    spawnExplosion(position, power, relativeAffectedBlocks, playerVelocity, null, null, null, null,
        null, viewers);
  }
  ...
  default void spawnExplosion(@NotNull Vector3d position,
      ...
      @Nullable List<Vector3i> relativeAffectedBlocks,
      @ArrayLenRange(from = 1) UUID @NotNull ... viewers) {
    spawnExplosion(position, power, relativeAffectedBlocks, null, null, null, null, null, null,
        viewers);
  }
   ...
   * @param value   the value of the game event (can be null - check if the type field is annotated
   *                with
  ...
  <T> void worldEvent(WorldEvent.Type<T> type, Vector3i position, @CheckForNull T value,
      UUID @NotNull ... viewers);

  <T> void worldEvent(WorldEvent.AllowBooleanType<T> type, Vector3i position, @CheckForNull T value,
      boolean global, UUID @NotNull ... viewers);

  default void worldEvent(WorldEvent.SimpleType type, Vector3i position,
      UUID @NotNull ... viewers) {
    worldEvent(type, position, null, viewers);
  }

  default <T> void worldEvent(WorldEvent.AllowBooleanType<T> type, Vector3i position,
      boolean global, UUID @NotNull ... viewers) {
