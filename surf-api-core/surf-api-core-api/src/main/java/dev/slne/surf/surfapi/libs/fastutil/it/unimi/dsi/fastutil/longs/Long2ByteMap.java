...
public interface Long2ByteMap extends Long2ByteFunction, Map<Long, Byte> {
	...
	// Defaultable methods
	@Override
	default void forEach(final java.util.function.BiConsumer<? super Long, ? super Byte> consumer) {
		final ObjectSet<Long2ByteMap.Entry> entrySet = long2ByteEntrySet();
		final Consumer<Long2ByteMap.Entry> wrappingConsumer = (entry) -> consumer.accept(Long.valueOf(entry.getLongKey()), Byte.valueOf(entry.getByteValue()));
	 ...
	 * @since 8.0.0
	 */
	default byte computeIfAbsent(final long key, final Long2ByteFunction mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final byte v = get(key), drv = defaultReturnValue();
		if (v != drv || containsKey(key)) return v;
	 ...
	 * @since 8.5.0
	 */
	default byte mergeByte(final long key, final byte value, final it.unimi.dsi.fastutil.bytes.ByteBinaryOperator remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final byte oldValue = get(key), drv = defaultReturnValue();
	 ...
	 *           this overhead.
	 */
	default byte mergeByte(final long key, final byte value, final java.util.function.IntBinaryOperator remappingFunction) {
