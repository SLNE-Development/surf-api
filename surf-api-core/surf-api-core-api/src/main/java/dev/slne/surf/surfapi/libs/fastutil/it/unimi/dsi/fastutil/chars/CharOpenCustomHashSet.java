	...
	public boolean add(final char k) {
		...
		if ((strategy.equals((k), ((char)0)))) {
			...
			if (!((curr = key[pos = (it.unimi.dsi.fastutil.HashCommon.mix(strategy.hashCode(k))) & mask]) == ((char)0))) {
				...
				while (!((curr = key[pos = (pos + 1) & mask]) == ((char)0))) if ((strategy.equals((curr), (k)))) return false;
			}
			key[pos] = k;
		}
	...
	private void readObject(java.io.ObjectInputStream s) throws java.io.IOException, ClassNotFoundException {
		...
		for (int i = size, pos; i-- != 0;) {
			...
			if ((strategy.equals((k), ((char)0)))) {
				...
				if (!((key[pos = (it.unimi.dsi.fastutil.HashCommon.mix(strategy.hashCode(k))) & mask]) == ((char)0))) while (!((key[pos = (pos + 1) & mask]) == ((char)0)));
			}
			key[pos] = k;
		}
		if (ASSERTS) checkTable();
