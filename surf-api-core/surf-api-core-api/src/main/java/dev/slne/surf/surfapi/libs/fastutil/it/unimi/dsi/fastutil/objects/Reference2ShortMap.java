...
public interface Reference2ShortMap<K> extends Reference2ShortFunction<K>, Map<K, Short> {
	...
	// Defaultable methods
	@Override
	default void forEach(final java.util.function.BiConsumer<? super K, ? super Short> consumer) {
		final ObjectSet<Reference2ShortMap.Entry<K>> entrySet = reference2ShortEntrySet();
		final Consumer<Reference2ShortMap.Entry<K>> wrappingConsumer = (entry) -> consumer.accept((entry.getKey()), Short.valueOf(entry.getShortValue()));
	 ...
	 * @since 8.0.0
	 */
	default short computeIfAbsent(final K key, final java.util.function.ToIntFunction<? super K> mappingFunction) {
	 ...
	 * @since 8.0.0
	 */
	default short computeIfAbsent(final K key, final Reference2ShortFunction<? super K> mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final short v = getShort(key), drv = defaultReturnValue();
		if (v != drv || containsKey(key)) return v;
	 ...
	 * @since 8.5.0
	 */
	default short mergeShort(final K key, final short value, final it.unimi.dsi.fastutil.shorts.ShortBinaryOperator remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final short oldValue = getShort(key), drv = defaultReturnValue();
		final short newValue = oldValue != drv || containsKey(key) ? remappingFunction.apply(oldValue, value) : value;
		put(key, newValue);
		return newValue;
	}
	 ...
	 *           this overhead.
	 */
	default short mergeShort(final K key, final short value, final java.util.function.IntBinaryOperator remappingFunction) {
		return mergeShort(key, value, remappingFunction instanceof it.unimi.dsi.fastutil.shorts.ShortBinaryOperator ? (it.unimi.dsi.fastutil.shorts.ShortBinaryOperator)remappingFunction : (x, y) -> it.unimi.dsi.fastutil.SafeMath.safeIntToShort(remappingFunction.applyAsInt(x, y)));
	}
