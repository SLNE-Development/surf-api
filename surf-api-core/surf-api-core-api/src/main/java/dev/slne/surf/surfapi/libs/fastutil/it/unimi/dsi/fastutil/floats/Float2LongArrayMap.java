	...
	private final class EntrySet extends AbstractObjectSet<Float2LongMap.Entry> implements FastEntrySet {
		...
		public ObjectIterator<Float2LongMap.Entry> iterator() {
			return new ObjectIterator<Float2LongMap.Entry>() {
				...
				public void forEachRemaining(final Consumer<? super Float2LongMap.Entry> action) {
					// Hoist containing class field ref into local
					final int max = size;
					while (next < max) {
						action.accept(new AbstractFloat2LongMap.BasicEntry(key[curr = next], value[next++]));
					}
				}
			};
		}
		...
		final class EntrySetSpliterator extends ObjectSpliterators.EarlyBindingSizeIndexBasedSpliterator<Float2LongMap.Entry> implements ObjectSpliterator<Float2LongMap.Entry> {
			...
			protected final Float2LongMap.Entry get(int location) {
				return new AbstractFloat2LongMap.BasicEntry(key[location], value[location]);
			}
		...
		public void forEach(final Consumer<? super Float2LongMap.Entry> action) {
			// Hoist containing class field ref into local
			for (int i = 0, max = size; i < max; ++i) {
				action.accept(new AbstractFloat2LongMap.BasicEntry(key[i], value[i]));
			}
		}
		...
		public void fastForEach(final Consumer<? super Float2LongMap.Entry> action) {
			final BasicEntry entry = new BasicEntry();
			// Hoist containing class field ref into local
			for (int i = 0, max = size; i < max; ++i) {
				entry.key = key[i];
