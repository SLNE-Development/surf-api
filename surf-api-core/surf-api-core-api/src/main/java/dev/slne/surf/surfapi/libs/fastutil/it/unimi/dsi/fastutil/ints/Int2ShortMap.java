...
public interface Int2ShortMap extends Int2ShortFunction, Map<Integer, Short> {
	...
	// Defaultable methods
	@Override
	default void forEach(final java.util.function.BiConsumer<? super Integer, ? super Short> consumer) {
		final ObjectSet<Int2ShortMap.Entry> entrySet = int2ShortEntrySet();
		final Consumer<Int2ShortMap.Entry> wrappingConsumer = (entry) -> consumer.accept(Integer.valueOf(entry.getIntKey()), Short.valueOf(entry.getShortValue()));
	 ...
	 * @since 8.0.0
	 */
	default short computeIfAbsent(final int key, final Int2ShortFunction mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final short v = get(key), drv = defaultReturnValue();
		if (v != drv || containsKey(key)) return v;
	 ...
	 * @since 8.5.0
	 */
	default short mergeShort(final int key, final short value, final it.unimi.dsi.fastutil.shorts.ShortBinaryOperator remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final short oldValue = get(key), drv = defaultReturnValue();
	 ...
	 *           this overhead.
	 */
	default short mergeShort(final int key, final short value, final java.util.function.IntBinaryOperator remappingFunction) {
