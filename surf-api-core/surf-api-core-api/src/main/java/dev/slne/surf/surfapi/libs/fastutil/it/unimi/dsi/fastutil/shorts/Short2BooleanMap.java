...
public interface Short2BooleanMap extends Short2BooleanFunction, Map<Short, Boolean> {
	...
	// Defaultable methods
	@Override
	default void forEach(final java.util.function.BiConsumer<? super Short, ? super Boolean> consumer) {
		final ObjectSet<Short2BooleanMap.Entry> entrySet = short2BooleanEntrySet();
		final Consumer<Short2BooleanMap.Entry> wrappingConsumer = (entry) -> consumer.accept(Short.valueOf(entry.getShortKey()), Boolean.valueOf(entry.getBooleanValue()));
	 ...
	 * @since 8.0.0
	 */
	default boolean computeIfAbsent(final short key, final Short2BooleanFunction mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final boolean v = get(key), drv = defaultReturnValue();
		if (v != drv || containsKey(key)) return v;
