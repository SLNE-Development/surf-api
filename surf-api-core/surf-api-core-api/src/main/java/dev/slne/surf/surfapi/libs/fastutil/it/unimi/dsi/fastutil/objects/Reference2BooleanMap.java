...
public interface Reference2BooleanMap<K> extends Reference2BooleanFunction<K>, Map<K, Boolean> {
	...
	default void forEach(final java.util.function.BiConsumer<? super K, ? super Boolean> consumer) {
		final ObjectSet<Reference2BooleanMap.Entry<K>> entrySet = reference2BooleanEntrySet();
		final Consumer<Reference2BooleanMap.Entry<K>> wrappingConsumer = (entry) -> consumer.accept((entry.getKey()), Boolean.valueOf(entry.getBooleanValue()));
	 ...
	 * @since 8.0.0
	 */
	default boolean computeIfAbsent(final K key, final Reference2BooleanFunction<? super K> mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final boolean v = getBoolean(key), drv = defaultReturnValue();
		if (v != drv || containsKey(key)) return v;
