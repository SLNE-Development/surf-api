...
public interface Float2LongMap extends Float2LongFunction, Map<Float, Long> {
	...
	// Defaultable methods
	@Override
	default void forEach(final java.util.function.BiConsumer<? super Float, ? super Long> consumer) {
		final ObjectSet<Float2LongMap.Entry> entrySet = float2LongEntrySet();
		final Consumer<Float2LongMap.Entry> wrappingConsumer = (entry) -> consumer.accept(Float.valueOf(entry.getFloatKey()), Long.valueOf(entry.getLongValue()));
	 ...
	 * @since 8.0.0
	 */
	default long computeIfAbsent(final float key, final Float2LongFunction mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final long v = get(key), drv = defaultReturnValue();
		if (v != drv || containsKey(key)) return v;
	 ...
	 * @since 8.5.0
	 */
	default long mergeLong(final float key, final long value, final java.util.function.LongBinaryOperator remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final long oldValue = get(key), drv = defaultReturnValue();
	 ...
	 * @since 8.5.1
	 */
	default long mergeLong(final float key, final long value, final it.unimi.dsi.fastutil.longs.LongBinaryOperator remappingFunction) {
