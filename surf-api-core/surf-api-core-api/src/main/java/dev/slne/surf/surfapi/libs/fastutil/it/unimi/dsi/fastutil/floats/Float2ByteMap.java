...
public interface Float2ByteMap extends Float2ByteFunction, Map<Float, Byte> {
	...
	// Defaultable methods
	@Override
	default void forEach(final java.util.function.BiConsumer<? super Float, ? super Byte> consumer) {
		final ObjectSet<Float2ByteMap.Entry> entrySet = float2ByteEntrySet();
		final Consumer<Float2ByteMap.Entry> wrappingConsumer = (entry) -> consumer.accept(Float.valueOf(entry.getFloatKey()), Byte.valueOf(entry.getByteValue()));
	 ...
	 * @since 8.0.0
	 */
	default byte computeIfAbsent(final float key, final Float2ByteFunction mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final byte v = get(key), drv = defaultReturnValue();
		if (v != drv || containsKey(key)) return v;
	 ...
	 * @since 8.5.0
	 */
	default byte mergeByte(final float key, final byte value, final it.unimi.dsi.fastutil.bytes.ByteBinaryOperator remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final byte oldValue = get(key), drv = defaultReturnValue();
	 ...
	 *           this overhead.
	 */
	default byte mergeByte(final float key, final byte value, final java.util.function.IntBinaryOperator remappingFunction) {
