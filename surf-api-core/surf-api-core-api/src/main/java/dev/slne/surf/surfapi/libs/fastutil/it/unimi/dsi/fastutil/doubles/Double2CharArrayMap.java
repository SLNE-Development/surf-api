	...
	private final class EntrySet extends AbstractObjectSet<Double2CharMap.Entry> implements FastEntrySet {
		...
		public ObjectIterator<Double2CharMap.Entry> iterator() {
			return new ObjectIterator<Double2CharMap.Entry>() {
				...
				public void forEachRemaining(final Consumer<? super Double2CharMap.Entry> action) {
					// Hoist containing class field ref into local
					final int max = size;
					while (next < max) {
						action.accept(new AbstractDouble2CharMap.BasicEntry(key[curr = next], value[next++]));
					}
				}
			};
		}
		...
		final class EntrySetSpliterator extends ObjectSpliterators.EarlyBindingSizeIndexBasedSpliterator<Double2CharMap.Entry> implements ObjectSpliterator<Double2CharMap.Entry> {
			...
			protected final Double2CharMap.Entry get(int location) {
				return new AbstractDouble2CharMap.BasicEntry(key[location], value[location]);
			}
		...
		public void forEach(final Consumer<? super Double2CharMap.Entry> action) {
			// Hoist containing class field ref into local
			for (int i = 0, max = size; i < max; ++i) {
				action.accept(new AbstractDouble2CharMap.BasicEntry(key[i], value[i]));
			}
		}
		...
		public void fastForEach(final Consumer<? super Double2CharMap.Entry> action) {
			final BasicEntry entry = new BasicEntry();
			// Hoist containing class field ref into local
			for (int i = 0, max = size; i < max; ++i) {
				entry.key = key[i];
