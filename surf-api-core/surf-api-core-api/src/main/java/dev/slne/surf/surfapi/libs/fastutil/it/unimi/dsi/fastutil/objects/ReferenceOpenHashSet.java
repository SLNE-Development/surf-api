...
public class ReferenceOpenHashSet<K> extends AbstractReferenceSet<K> implements java.io.Serializable, Cloneable, Hash {
	...
	public static <K> ReferenceOpenHashSet<K> of(final K e) {
		ReferenceOpenHashSet<K> result = new ReferenceOpenHashSet<>(1, DEFAULT_LOAD_FACTOR);
		result.add(e);
		return result;
	}
	...
	public static <K> ReferenceOpenHashSet<K> of(final K e0, final K e1) {
		ReferenceOpenHashSet<K> result = new ReferenceOpenHashSet<>(2, DEFAULT_LOAD_FACTOR);
		result.add(e0);
		if (!result.add(e1)) {
	...
	public static <K> ReferenceOpenHashSet<K> of(final K e0, final K e1, final K e2) {
		ReferenceOpenHashSet<K> result = new ReferenceOpenHashSet<>(3, DEFAULT_LOAD_FACTOR);
		result.add(e0);
		if (!result.add(e1)) {
			throw new IllegalArgumentException("Duplicate element: " + e1);
		}
	...
	public static <K> ReferenceOpenHashSet<K> of(final K... a) {
		ReferenceOpenHashSet<K> result = new ReferenceOpenHashSet<>(a.length, DEFAULT_LOAD_FACTOR);
		for (K element : a) {
	...
	public static <K> Collector<K, ?, ReferenceOpenHashSet<K>> toSetWithExpectedSize(int expectedSize) {
		...
		return Collector.of(new ReferenceCollections.SizeDecreasingSupplier<K, ReferenceOpenHashSet<K>>(expectedSize, (int size) -> size <= Hash.DEFAULT_INITIAL_SIZE ? new ReferenceOpenHashSet<K>() : new ReferenceOpenHashSet<K>(size)), ReferenceOpenHashSet::add, ReferenceOpenHashSet::combine, Collector.Characteristics.UNORDERED);
	}
	...
	public boolean add(final K k) {
		...
		if (((k) == (null))) {
			...
			if (!((curr = key[pos = (it.unimi.dsi.fastutil.HashCommon.mix(System.identityHashCode(k))) & mask]) == (null))) {
				...
				while (!((curr = key[pos = (pos + 1) & mask]) == (null))) if (((curr) == (k))) return false;
			}
			key[pos] = k;
		}
