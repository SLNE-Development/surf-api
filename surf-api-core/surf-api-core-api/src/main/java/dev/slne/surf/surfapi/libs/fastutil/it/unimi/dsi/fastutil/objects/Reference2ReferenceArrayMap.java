	...
	private final class EntrySet extends AbstractObjectSet<Reference2ReferenceMap.Entry<K, V>> implements FastEntrySet<K, V> {
		...
		public ObjectIterator<Reference2ReferenceMap.Entry<K, V>> iterator() {
			return new ObjectIterator<Reference2ReferenceMap.Entry<K, V>>() {
				...
				@Override
				@SuppressWarnings("unchecked")
				public void forEachRemaining(final Consumer<? super Reference2ReferenceMap.Entry<K, V>> action) {
					// Hoist containing class field ref into local
					final int max = size;
					while (next < max) {
						action.accept(new AbstractReference2ReferenceMap.BasicEntry<>((K)key[curr = next], (V)value[next++]));
					}
				}
			};
		}
		...
		final class EntrySetSpliterator extends ObjectSpliterators.EarlyBindingSizeIndexBasedSpliterator<Reference2ReferenceMap.Entry<K, V>> implements ObjectSpliterator<Reference2ReferenceMap.Entry<K, V>> {
			...
			protected final Reference2ReferenceMap.Entry<K, V> get(int location) {
				return new AbstractReference2ReferenceMap.BasicEntry<>((K)key[location], (V)value[location]);
			}
		...
		@Override
		@SuppressWarnings("unchecked")
		public void forEach(final Consumer<? super Reference2ReferenceMap.Entry<K, V>> action) {
			// Hoist containing class field ref into local
			for (int i = 0, max = size; i < max; ++i) {
				action.accept(new AbstractReference2ReferenceMap.BasicEntry<>((K)key[i], (V)value[i]));
			}
		}
		...
		@Override
		@SuppressWarnings("unchecked")
		public void fastForEach(final Consumer<? super Reference2ReferenceMap.Entry<K, V>> action) {
			final BasicEntry<K, V> entry = new BasicEntry<>();
			// Hoist containing class field ref into local
			for (int i = 0, max = size; i < max; ++i) {
				entry.key = (K)key[i];
