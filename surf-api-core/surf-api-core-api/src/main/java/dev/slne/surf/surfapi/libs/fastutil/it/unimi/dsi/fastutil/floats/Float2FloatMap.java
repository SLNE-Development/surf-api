...
public interface Float2FloatMap extends Float2FloatFunction, Map<Float, Float> {
	...
	// Defaultable methods
	@Override
	default void forEach(final java.util.function.BiConsumer<? super Float, ? super Float> consumer) {
		final ObjectSet<Float2FloatMap.Entry> entrySet = float2FloatEntrySet();
		final Consumer<Float2FloatMap.Entry> wrappingConsumer = (entry) -> consumer.accept(Float.valueOf(entry.getFloatKey()), Float.valueOf(entry.getFloatValue()));
	 ...
	 * @since 8.0.0
	 */
	default float computeIfAbsent(final float key, final Float2FloatFunction mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final float v = get(key), drv = defaultReturnValue();
		if (v != drv || containsKey(key)) return v;
	 ...
	 * @since 8.5.0
	 */
	default float mergeFloat(final float key, final float value, final it.unimi.dsi.fastutil.floats.FloatBinaryOperator remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final float oldValue = get(key), drv = defaultReturnValue();
	 ...
	 *           this overhead.
	 */
	default float mergeFloat(final float key, final float value, final java.util.function.DoubleBinaryOperator remappingFunction) {
