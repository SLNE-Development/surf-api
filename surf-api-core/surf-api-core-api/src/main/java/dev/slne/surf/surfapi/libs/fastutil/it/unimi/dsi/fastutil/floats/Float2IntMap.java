...
public interface Float2IntMap extends Float2IntFunction, Map<Float, Integer> {
	...
	// Defaultable methods
	@Override
	default void forEach(final java.util.function.BiConsumer<? super Float, ? super Integer> consumer) {
		final ObjectSet<Float2IntMap.Entry> entrySet = float2IntEntrySet();
		final Consumer<Float2IntMap.Entry> wrappingConsumer = (entry) -> consumer.accept(Float.valueOf(entry.getFloatKey()), Integer.valueOf(entry.getIntValue()));
	 ...
	 * @since 8.0.0
	 */
	default int computeIfAbsent(final float key, final Float2IntFunction mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final int v = get(key), drv = defaultReturnValue();
		if (v != drv || containsKey(key)) return v;
	 ...
	 * @since 8.5.0
	 */
	default int mergeInt(final float key, final int value, final java.util.function.IntBinaryOperator remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int oldValue = get(key), drv = defaultReturnValue();
	 ...
	 * @since 8.5.1
	 */
	default int mergeInt(final float key, final int value, final it.unimi.dsi.fastutil.ints.IntBinaryOperator remappingFunction) {
