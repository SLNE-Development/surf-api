	...
	private final class EntrySet extends AbstractObjectSet<Int2ByteMap.Entry> implements FastEntrySet {
		...
		public ObjectIterator<Int2ByteMap.Entry> iterator() {
			return new ObjectIterator<Int2ByteMap.Entry>() {
				...
				public void forEachRemaining(final Consumer<? super Int2ByteMap.Entry> action) {
					// Hoist containing class field ref into local
					final int max = size;
					while (next < max) {
						action.accept(new AbstractInt2ByteMap.BasicEntry(key[curr = next], value[next++]));
					}
				}
			};
		}
		...
		final class EntrySetSpliterator extends ObjectSpliterators.EarlyBindingSizeIndexBasedSpliterator<Int2ByteMap.Entry> implements ObjectSpliterator<Int2ByteMap.Entry> {
			...
			protected final Int2ByteMap.Entry get(int location) {
				return new AbstractInt2ByteMap.BasicEntry(key[location], value[location]);
			}
		...
		public void forEach(final Consumer<? super Int2ByteMap.Entry> action) {
			// Hoist containing class field ref into local
			for (int i = 0, max = size; i < max; ++i) {
				action.accept(new AbstractInt2ByteMap.BasicEntry(key[i], value[i]));
			}
		}
		...
		public void fastForEach(final Consumer<? super Int2ByteMap.Entry> action) {
			final BasicEntry entry = new BasicEntry();
			// Hoist containing class field ref into local
			for (int i = 0, max = size; i < max; ++i) {
				entry.key = key[i];
