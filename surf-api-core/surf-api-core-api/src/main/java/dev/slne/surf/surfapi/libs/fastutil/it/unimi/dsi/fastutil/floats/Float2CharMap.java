...
public interface Float2CharMap extends Float2CharFunction, Map<Float, Character> {
	...
	// Defaultable methods
	@Override
	default void forEach(final java.util.function.BiConsumer<? super Float, ? super Character> consumer) {
		final ObjectSet<Float2CharMap.Entry> entrySet = float2CharEntrySet();
		final Consumer<Float2CharMap.Entry> wrappingConsumer = (entry) -> consumer.accept(Float.valueOf(entry.getFloatKey()), Character.valueOf(entry.getCharValue()));
	 ...
	 * @since 8.0.0
	 */
	default char computeIfAbsent(final float key, final Float2CharFunction mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final char v = get(key), drv = defaultReturnValue();
		if (v != drv || containsKey(key)) return v;
	 ...
	 * @since 8.5.0
	 */
	default char mergeChar(final float key, final char value, final it.unimi.dsi.fastutil.chars.CharBinaryOperator remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final char oldValue = get(key), drv = defaultReturnValue();
	 ...
	 *           this overhead.
	 */
	default char mergeChar(final float key, final char value, final java.util.function.IntBinaryOperator remappingFunction) {
