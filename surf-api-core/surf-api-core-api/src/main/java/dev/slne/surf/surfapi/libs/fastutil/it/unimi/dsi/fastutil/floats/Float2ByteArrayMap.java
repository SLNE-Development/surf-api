	...
	private final class EntrySet extends AbstractObjectSet<Float2ByteMap.Entry> implements FastEntrySet {
		...
		public ObjectIterator<Float2ByteMap.Entry> iterator() {
			return new ObjectIterator<Float2ByteMap.Entry>() {
				...
				public void forEachRemaining(final Consumer<? super Float2ByteMap.Entry> action) {
					// Hoist containing class field ref into local
					final int max = size;
					while (next < max) {
						action.accept(new AbstractFloat2ByteMap.BasicEntry(key[curr = next], value[next++]));
					}
				}
			};
		}
		...
		final class EntrySetSpliterator extends ObjectSpliterators.EarlyBindingSizeIndexBasedSpliterator<Float2ByteMap.Entry> implements ObjectSpliterator<Float2ByteMap.Entry> {
			...
			protected final Float2ByteMap.Entry get(int location) {
				return new AbstractFloat2ByteMap.BasicEntry(key[location], value[location]);
			}
		...
		public void forEach(final Consumer<? super Float2ByteMap.Entry> action) {
			// Hoist containing class field ref into local
			for (int i = 0, max = size; i < max; ++i) {
				action.accept(new AbstractFloat2ByteMap.BasicEntry(key[i], value[i]));
			}
		}
		...
		public void fastForEach(final Consumer<? super Float2ByteMap.Entry> action) {
			final BasicEntry entry = new BasicEntry();
			// Hoist containing class field ref into local
			for (int i = 0, max = size; i < max; ++i) {
				entry.key = key[i];
