...
public interface Short2ObjectMap<V> extends Short2ObjectFunction<V>, Map<Short, V> {
	...
	default void forEach(final java.util.function.BiConsumer<? super Short, ? super V> consumer) {
		final ObjectSet<Short2ObjectMap.Entry<V>> entrySet = short2ObjectEntrySet();
		final Consumer<Short2ObjectMap.Entry<V>> wrappingConsumer = (entry) -> consumer.accept(Short.valueOf(entry.getShortKey()), (entry.getValue()));
	 ...
	 * @since 8.0.0
	 */
	default V computeIfAbsent(final short key, final Short2ObjectFunction<? extends V> mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final V v = get(key), drv = defaultReturnValue();
		if (v != drv || containsKey(key)) return v;
