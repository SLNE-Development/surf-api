...
public interface Float2BooleanMap extends Float2BooleanFunction, Map<Float, Boolean> {
	...
	// Defaultable methods
	@Override
	default void forEach(final java.util.function.BiConsumer<? super Float, ? super Boolean> consumer) {
		final ObjectSet<Float2BooleanMap.Entry> entrySet = float2BooleanEntrySet();
		final Consumer<Float2BooleanMap.Entry> wrappingConsumer = (entry) -> consumer.accept(Float.valueOf(entry.getFloatKey()), Boolean.valueOf(entry.getBooleanValue()));
	 ...
	 * @since 8.0.0
	 */
	default boolean computeIfAbsent(final float key, final Float2BooleanFunction mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final boolean v = get(key), drv = defaultReturnValue();
		if (v != drv || containsKey(key)) return v;
