	...
	private final class EntrySet extends AbstractObjectSet<Long2ShortMap.Entry> implements FastEntrySet {
		...
		public ObjectIterator<Long2ShortMap.Entry> iterator() {
			return new ObjectIterator<Long2ShortMap.Entry>() {
				...
				public void forEachRemaining(final Consumer<? super Long2ShortMap.Entry> action) {
					// Hoist containing class field ref into local
					final int max = size;
					while (next < max) {
						action.accept(new AbstractLong2ShortMap.BasicEntry(key[curr = next], value[next++]));
					}
				}
			};
		}
		...
		final class EntrySetSpliterator extends ObjectSpliterators.EarlyBindingSizeIndexBasedSpliterator<Long2ShortMap.Entry> implements ObjectSpliterator<Long2ShortMap.Entry> {
			...
			protected final Long2ShortMap.Entry get(int location) {
				return new AbstractLong2ShortMap.BasicEntry(key[location], value[location]);
			}
		...
		public void forEach(final Consumer<? super Long2ShortMap.Entry> action) {
			// Hoist containing class field ref into local
			for (int i = 0, max = size; i < max; ++i) {
				action.accept(new AbstractLong2ShortMap.BasicEntry(key[i], value[i]));
			}
		}
		...
		public void fastForEach(final Consumer<? super Long2ShortMap.Entry> action) {
			final BasicEntry entry = new BasicEntry();
			// Hoist containing class field ref into local
			for (int i = 0, max = size; i < max; ++i) {
				entry.key = key[i];
