...
public interface Short2DoubleMap extends Short2DoubleFunction, Map<Short, Double> {
	...
	// Defaultable methods
	@Override
	default void forEach(final java.util.function.BiConsumer<? super Short, ? super Double> consumer) {
		final ObjectSet<Short2DoubleMap.Entry> entrySet = short2DoubleEntrySet();
		final Consumer<Short2DoubleMap.Entry> wrappingConsumer = (entry) -> consumer.accept(Short.valueOf(entry.getShortKey()), Double.valueOf(entry.getDoubleValue()));
	 ...
	 * @since 8.0.0
	 */
	default double computeIfAbsent(final short key, final Short2DoubleFunction mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final double v = get(key), drv = defaultReturnValue();
		if (v != drv || containsKey(key)) return v;
	 ...
	 * @since 8.5.0
	 */
	default double mergeDouble(final short key, final double value, final java.util.function.DoubleBinaryOperator remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final double oldValue = get(key), drv = defaultReturnValue();
	 ...
	 * @since 8.5.1
	 */
	default double mergeDouble(final short key, final double value, final it.unimi.dsi.fastutil.doubles.DoubleBinaryOperator remappingFunction) {
