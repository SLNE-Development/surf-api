	...
	private final class EntrySet extends AbstractObjectSet<Long2ByteMap.Entry> implements FastEntrySet {
		...
		public ObjectIterator<Long2ByteMap.Entry> iterator() {
			return new ObjectIterator<Long2ByteMap.Entry>() {
				...
				public void forEachRemaining(final Consumer<? super Long2ByteMap.Entry> action) {
					// Hoist containing class field ref into local
					final int max = size;
					while (next < max) {
						action.accept(new AbstractLong2ByteMap.BasicEntry(key[curr = next], value[next++]));
					}
				}
			};
		}
		...
		final class EntrySetSpliterator extends ObjectSpliterators.EarlyBindingSizeIndexBasedSpliterator<Long2ByteMap.Entry> implements ObjectSpliterator<Long2ByteMap.Entry> {
			...
			protected final Long2ByteMap.Entry get(int location) {
				return new AbstractLong2ByteMap.BasicEntry(key[location], value[location]);
			}
		...
		public void forEach(final Consumer<? super Long2ByteMap.Entry> action) {
			// Hoist containing class field ref into local
			for (int i = 0, max = size; i < max; ++i) {
				action.accept(new AbstractLong2ByteMap.BasicEntry(key[i], value[i]));
			}
		}
		...
		public void fastForEach(final Consumer<? super Long2ByteMap.Entry> action) {
			final BasicEntry entry = new BasicEntry();
			// Hoist containing class field ref into local
			for (int i = 0, max = size; i < max; ++i) {
				entry.key = key[i];
