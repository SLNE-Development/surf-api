...
public interface Int2CharMap extends Int2CharFunction, Map<Integer, Character> {
	...
	// Defaultable methods
	@Override
	default void forEach(final java.util.function.BiConsumer<? super Integer, ? super Character> consumer) {
		final ObjectSet<Int2CharMap.Entry> entrySet = int2CharEntrySet();
		final Consumer<Int2CharMap.Entry> wrappingConsumer = (entry) -> consumer.accept(Integer.valueOf(entry.getIntKey()), Character.valueOf(entry.getCharValue()));
	 ...
	 * @since 8.0.0
	 */
	default char computeIfAbsent(final int key, final Int2CharFunction mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final char v = get(key), drv = defaultReturnValue();
		if (v != drv || containsKey(key)) return v;
	 ...
	 * @since 8.5.0
	 */
	default char mergeChar(final int key, final char value, final it.unimi.dsi.fastutil.chars.CharBinaryOperator remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final char oldValue = get(key), drv = defaultReturnValue();
	 ...
	 *           this overhead.
	 */
	default char mergeChar(final int key, final char value, final java.util.function.IntBinaryOperator remappingFunction) {
