	...
	private final class EntrySet extends AbstractObjectSet<Byte2ReferenceMap.Entry<V>> implements FastEntrySet<V> {
		...
		public ObjectIterator<Byte2ReferenceMap.Entry<V>> iterator() {
			return new ObjectIterator<Byte2ReferenceMap.Entry<V>>() {
				...
				@Override
				@SuppressWarnings("unchecked")
				public void forEachRemaining(final Consumer<? super Byte2ReferenceMap.Entry<V>> action) {
					// Hoist containing class field ref into local
					final int max = size;
					while (next < max) {
						action.accept(new AbstractByte2ReferenceMap.BasicEntry<>(key[curr = next], (V)value[next++]));
					}
				}
			};
		}
		...
		final class EntrySetSpliterator extends ObjectSpliterators.EarlyBindingSizeIndexBasedSpliterator<Byte2ReferenceMap.Entry<V>> implements ObjectSpliterator<Byte2ReferenceMap.Entry<V>> {
			...
			protected final Byte2ReferenceMap.Entry<V> get(int location) {
				return new AbstractByte2ReferenceMap.BasicEntry<>(key[location], (V)value[location]);
			}
		...
		@Override
		@SuppressWarnings("unchecked")
		public void forEach(final Consumer<? super Byte2ReferenceMap.Entry<V>> action) {
			// Hoist containing class field ref into local
			for (int i = 0, max = size; i < max; ++i) {
				action.accept(new AbstractByte2ReferenceMap.BasicEntry<>(key[i], (V)value[i]));
			}
		}
		...
		@Override
		@SuppressWarnings("unchecked")
		public void fastForEach(final Consumer<? super Byte2ReferenceMap.Entry<V>> action) {
			final BasicEntry<V> entry = new BasicEntry<>();
			// Hoist containing class field ref into local
			for (int i = 0, max = size; i < max; ++i) {
				entry.key = key[i];
