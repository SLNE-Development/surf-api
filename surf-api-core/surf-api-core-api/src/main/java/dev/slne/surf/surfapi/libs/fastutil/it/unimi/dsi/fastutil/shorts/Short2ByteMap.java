...
public interface Short2ByteMap extends Short2ByteFunction, Map<Short, Byte> {
	...
	// Defaultable methods
	@Override
	default void forEach(final java.util.function.BiConsumer<? super Short, ? super Byte> consumer) {
		final ObjectSet<Short2ByteMap.Entry> entrySet = short2ByteEntrySet();
		final Consumer<Short2ByteMap.Entry> wrappingConsumer = (entry) -> consumer.accept(Short.valueOf(entry.getShortKey()), Byte.valueOf(entry.getByteValue()));
	 ...
	 * @since 8.0.0
	 */
	default byte computeIfAbsent(final short key, final Short2ByteFunction mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final byte v = get(key), drv = defaultReturnValue();
		if (v != drv || containsKey(key)) return v;
	 ...
	 * @since 8.5.0
	 */
	default byte mergeByte(final short key, final byte value, final it.unimi.dsi.fastutil.bytes.ByteBinaryOperator remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final byte oldValue = get(key), drv = defaultReturnValue();
	 ...
	 *           this overhead.
	 */
	default byte mergeByte(final short key, final byte value, final java.util.function.IntBinaryOperator remappingFunction) {
