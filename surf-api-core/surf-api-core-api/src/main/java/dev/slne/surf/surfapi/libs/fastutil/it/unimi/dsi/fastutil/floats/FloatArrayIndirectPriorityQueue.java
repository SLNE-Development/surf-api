...
public class FloatArrayIndirectPriorityQueue implements FloatIndirectPriorityQueue {
	...
	private int findFirst() {
		...
		if (c == null) while (i-- != 0) {
			if ((Float.compare((refArray[array[i]]), (first)) < 0)) first = refArray[array[firstIndex = i]];
		}
		else while (i-- != 0) {
	...
	private int findLast() {
		...
		if (c == null) {
			while (i-- != 0) if ((Float.compare((last), (refArray[array[i]])) < 0)) last = refArray[array[lastIndex = i]];
		} else {
	...
	public void enqueue(int x) {
		...
		if (firstIndexValid) {
			if (c == null) {
				if ((Float.compare((refArray[x]), (refArray[array[firstIndex]])) < 0)) firstIndex = size;
