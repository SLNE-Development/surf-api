...
public class Reference2ShortArrayMap<K> extends AbstractReference2ShortMap<K> implements java.io.Serializable, Cloneable {
	...
	public Reference2ShortArrayMap(final Map<? extends K, ? extends Short> m) {
		this(m.size());
		int i = 0;
		for (Map.Entry<? extends K, ? extends Short> e : m.entrySet()) {
			key[i] = (e.getKey());
	...
	private final class EntrySet extends AbstractObjectSet<Reference2ShortMap.Entry<K>> implements FastEntrySet<K> {
		...
		public ObjectIterator<Reference2ShortMap.Entry<K>> iterator() {
			return new ObjectIterator<Reference2ShortMap.Entry<K>>() {
				...
				@Override
				@SuppressWarnings("unchecked")
				public void forEachRemaining(final Consumer<? super Reference2ShortMap.Entry<K>> action) {
					// Hoist containing class field ref into local
					final int max = size;
					while (next < max) {
						action.accept(new AbstractReference2ShortMap.BasicEntry<>((K)key[curr = next], value[next++]));
					}
				}
			};
		}
		...
		final class EntrySetSpliterator extends ObjectSpliterators.EarlyBindingSizeIndexBasedSpliterator<Reference2ShortMap.Entry<K>> implements ObjectSpliterator<Reference2ShortMap.Entry<K>> {
			...
			protected final Reference2ShortMap.Entry<K> get(int location) {
				return new AbstractReference2ShortMap.BasicEntry<>((K)key[location], value[location]);
			}
		...
		@Override
		@SuppressWarnings("unchecked")
		public void forEach(final Consumer<? super Reference2ShortMap.Entry<K>> action) {
			// Hoist containing class field ref into local
			for (int i = 0, max = size; i < max; ++i) {
				action.accept(new AbstractReference2ShortMap.BasicEntry<>((K)key[i], value[i]));
			}
		}
		...
		@Override
		@SuppressWarnings("unchecked")
		public void fastForEach(final Consumer<? super Reference2ShortMap.Entry<K>> action) {
			final BasicEntry<K> entry = new BasicEntry<>();
			// Hoist containing class field ref into local
			for (int i = 0, max = size; i < max; ++i) {
				entry.key = (K)key[i];
