...
public class Float2ReferenceOpenCustomHashMap<V> extends AbstractFloat2ReferenceMap<V> implements java.io.Serializable, Cloneable, Hash {
	...
	private abstract class MapSpliterator<ConsumerType, SplitType extends MapSpliterator<ConsumerType, SplitType>> {
		...
		boolean mustReturnNull = Float2ReferenceOpenCustomHashMap.this.containsNullKey;
	...
	protected void rehash(final int newN) {
		...
		for (int j = realSize(); j-- != 0;) {
			...
			if (!(Float.floatToIntBits(newKey[pos = (it.unimi.dsi.fastutil.HashCommon.mix(strategy.hashCode(key[i]))) & mask]) == 0)) while (!(Float.floatToIntBits(newKey[pos = (pos + 1) & mask]) == 0));
			newKey[pos] = key[i];
