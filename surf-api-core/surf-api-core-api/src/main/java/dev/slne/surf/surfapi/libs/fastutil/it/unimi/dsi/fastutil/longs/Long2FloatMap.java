...
public interface Long2FloatMap extends Long2FloatFunction, Map<Long, Float> {
	...
	// Defaultable methods
	@Override
	default void forEach(final java.util.function.BiConsumer<? super Long, ? super Float> consumer) {
		final ObjectSet<Long2FloatMap.Entry> entrySet = long2FloatEntrySet();
		final Consumer<Long2FloatMap.Entry> wrappingConsumer = (entry) -> consumer.accept(Long.valueOf(entry.getLongKey()), Float.valueOf(entry.getFloatValue()));
	 ...
	 * @since 8.0.0
	 */
	default float computeIfAbsent(final long key, final Long2FloatFunction mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final float v = get(key), drv = defaultReturnValue();
		if (v != drv || containsKey(key)) return v;
	 ...
	 * @since 8.5.0
	 */
	default float mergeFloat(final long key, final float value, final it.unimi.dsi.fastutil.floats.FloatBinaryOperator remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final float oldValue = get(key), drv = defaultReturnValue();
	 ...
	 *           this overhead.
	 */
	default float mergeFloat(final long key, final float value, final java.util.function.DoubleBinaryOperator remappingFunction) {
