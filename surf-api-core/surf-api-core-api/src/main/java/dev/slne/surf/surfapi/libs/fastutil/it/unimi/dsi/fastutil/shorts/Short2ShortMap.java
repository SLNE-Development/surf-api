...
public interface Short2ShortMap extends Short2ShortFunction, Map<Short, Short> {
	...
	// Defaultable methods
	@Override
	default void forEach(final java.util.function.BiConsumer<? super Short, ? super Short> consumer) {
		final ObjectSet<Short2ShortMap.Entry> entrySet = short2ShortEntrySet();
		final Consumer<Short2ShortMap.Entry> wrappingConsumer = (entry) -> consumer.accept(Short.valueOf(entry.getShortKey()), Short.valueOf(entry.getShortValue()));
	 ...
	 * @since 8.0.0
	 */
	default short computeIfAbsent(final short key, final Short2ShortFunction mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final short v = get(key), drv = defaultReturnValue();
		if (v != drv || containsKey(key)) return v;
	 ...
	 * @since 8.5.0
	 */
	default short mergeShort(final short key, final short value, final it.unimi.dsi.fastutil.shorts.ShortBinaryOperator remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final short oldValue = get(key), drv = defaultReturnValue();
	 ...
	 *           this overhead.
	 */
	default short mergeShort(final short key, final short value, final java.util.function.IntBinaryOperator remappingFunction) {
