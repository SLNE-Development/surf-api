...
public interface Float2ObjectMap<V> extends Float2ObjectFunction<V>, Map<Float, V> {
	...
	default void forEach(final java.util.function.BiConsumer<? super Float, ? super V> consumer) {
		final ObjectSet<Float2ObjectMap.Entry<V>> entrySet = float2ObjectEntrySet();
		final Consumer<Float2ObjectMap.Entry<V>> wrappingConsumer = (entry) -> consumer.accept(Float.valueOf(entry.getFloatKey()), (entry.getValue()));
	 ...
	 * @since 8.0.0
	 */
	default V computeIfAbsent(final float key, final Float2ObjectFunction<? extends V> mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final V v = get(key), drv = defaultReturnValue();
		if (v != drv || containsKey(key)) return v;
