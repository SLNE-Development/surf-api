...
public interface ByteCollection extends Collection<Byte>, ByteIterable {
	...
	default ByteSpliterator spliterator() {
		return ByteSpliterators.asSpliterator(iterator(), sizeOf(this), ByteSpliterators.COLLECTION_SPLITERATOR_CHARACTERISTICS);
	}
	...
	default boolean removeIf(final java.util.function.Predicate<? super Byte> filter) {
		return removeIf(filter instanceof BytePredicate ? ((BytePredicate)filter) : (BytePredicate)key -> filter.test(Byte.valueOf(it.unimi.dsi.fastutil.SafeMath.safeIntToByte(key))));
	}
	 ...
	 *          conversions).
	 */
	default boolean removeIf(final BytePredicate filter) {
	...
	default boolean removeIf(final java.util.function.IntPredicate filter) {
		return removeIf(filter instanceof BytePredicate ? (BytePredicate)filter : (BytePredicate)filter::test);
	}
	...
	@Deprecated
	@Override
	default java.util.stream.Stream<Byte> stream() {
	...
	@Deprecated
	@Override
	default java.util.stream.Stream<Byte> parallelStream() {
