...
public class ReferenceArraySet<K> extends AbstractReferenceSet<K> implements java.io.Serializable, Cloneable {
	 ...
	 * @param c a collection.
	 */
	public ReferenceArraySet(final Set<? extends K> c) {
		this(c.size());
		int i = 0;
		for (K x : c) {
			a[i] = (x);
	 ...
	 */
	public static <K> ReferenceArraySet<K> of(final K e) {
	 ...
	 */
	@SafeVarargs
	public static <K> ReferenceArraySet<K> of(final K... a) {
		if (a.length == 2) {
			if (((a[0]) == (a[1]))) {
	 ...
	 */
	@SafeVarargs
	public static <K> ReferenceArraySet<K> ofUnchecked(final K... a) {
	...
	private final class Spliterator implements ObjectSpliterator<K> {
		...
		private int getWorkingMax() {
			return hasSplit ? max : ReferenceArraySet.this.size;
		}
