...
public interface CharSet extends CharCollection, Set<Character> {
	...
	default CharSpliterator spliterator() {
		return CharSpliterators.asSpliterator(iterator(), sizeOf(this), CharSpliterators.SET_SPLITERATOR_CHARACTERISTICS);
	}
	...
	public static CharSet of(char... a) {
		...
		CharSet innerSet = a.length <= CharSets.ARRAY_SET_CUTOFF ? new CharArraySet(a.length) : new CharOpenHashSet(a.length);
		for (char element : a) {
