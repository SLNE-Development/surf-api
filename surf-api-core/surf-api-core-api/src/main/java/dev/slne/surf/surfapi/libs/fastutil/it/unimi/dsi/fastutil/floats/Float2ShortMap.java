...
public interface Float2ShortMap extends Float2ShortFunction, Map<Float, Short> {
	...
	// Defaultable methods
	@Override
	default void forEach(final java.util.function.BiConsumer<? super Float, ? super Short> consumer) {
		final ObjectSet<Float2ShortMap.Entry> entrySet = float2ShortEntrySet();
		final Consumer<Float2ShortMap.Entry> wrappingConsumer = (entry) -> consumer.accept(Float.valueOf(entry.getFloatKey()), Short.valueOf(entry.getShortValue()));
	 ...
	 * @since 8.0.0
	 */
	default short computeIfAbsent(final float key, final Float2ShortFunction mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final short v = get(key), drv = defaultReturnValue();
		if (v != drv || containsKey(key)) return v;
	 ...
	 * @since 8.5.0
	 */
	default short mergeShort(final float key, final short value, final it.unimi.dsi.fastutil.shorts.ShortBinaryOperator remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final short oldValue = get(key), drv = defaultReturnValue();
	 ...
	 *           this overhead.
	 */
	default short mergeShort(final float key, final short value, final java.util.function.IntBinaryOperator remappingFunction) {
