...
public interface Int2BooleanMap extends Int2BooleanFunction, Map<Integer, Boolean> {
	...
	// Defaultable methods
	@Override
	default void forEach(final java.util.function.BiConsumer<? super Integer, ? super Boolean> consumer) {
		final ObjectSet<Int2BooleanMap.Entry> entrySet = int2BooleanEntrySet();
		final Consumer<Int2BooleanMap.Entry> wrappingConsumer = (entry) -> consumer.accept(Integer.valueOf(entry.getIntKey()), Boolean.valueOf(entry.getBooleanValue()));
	 ...
	 * @since 8.0.0
	 */
	default boolean computeIfAbsent(final int key, final Int2BooleanFunction mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final boolean v = get(key), drv = defaultReturnValue();
		if (v != drv || containsKey(key)) return v;
