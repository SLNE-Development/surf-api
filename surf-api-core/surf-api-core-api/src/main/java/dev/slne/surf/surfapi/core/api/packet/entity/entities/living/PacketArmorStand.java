...
public interface PacketArmorStand extends PacketLivingEntity<PacketArmorStand>, Spawnable {
  ...
  interface Poses {
     ...
     * @param onlyModifyRotation if only the rotation should be modified and not e.g., attributes or
     *                           equipment of the armor stand
     * @param pushBatchUpdate    if the pose should be applied immediately (Set to {@code false} if
     *                           the entity is already in {@link PacketEntity#startBatchUpdate()}
     */
    ...
    /**
     * Applies the pose without any modifications other than rotation to the provided armor stand
     * immediately.
     *
     * @param armorStand the armor stand to apply the pose to
