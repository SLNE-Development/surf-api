	...
	private final class EntrySet extends AbstractObjectSet<Int2IntMap.Entry> implements FastEntrySet {
		...
		public ObjectIterator<Int2IntMap.Entry> iterator() {
			return new ObjectIterator<Int2IntMap.Entry>() {
				...
				public void forEachRemaining(final Consumer<? super Int2IntMap.Entry> action) {
					...
					final int max = size;
					while (next < max) {
						action.accept(new AbstractInt2IntMap.BasicEntry(key[curr = next], value[next++]));
					}
				}
			};
		}
		...
		final class EntrySetSpliterator extends ObjectSpliterators.EarlyBindingSizeIndexBasedSpliterator<Int2IntMap.Entry> implements ObjectSpliterator<Int2IntMap.Entry> {
			...
			protected final Int2IntMap.Entry get(int location) {
				return new AbstractInt2IntMap.BasicEntry(key[location], value[location]);
			}
		...
		public void forEach(final Consumer<? super Int2IntMap.Entry> action) {
			...
			for (int i = 0, max = size; i < max; ++i) {
				action.accept(new AbstractInt2IntMap.BasicEntry(key[i], value[i]));
			}
		}
