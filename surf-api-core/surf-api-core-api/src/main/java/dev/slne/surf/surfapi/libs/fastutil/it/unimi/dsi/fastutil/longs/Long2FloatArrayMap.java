	...
	private final class EntrySet extends AbstractObjectSet<Long2FloatMap.Entry> implements FastEntrySet {
		...
		public ObjectIterator<Long2FloatMap.Entry> iterator() {
			return new ObjectIterator<Long2FloatMap.Entry>() {
				...
				public void forEachRemaining(final Consumer<? super Long2FloatMap.Entry> action) {
					// Hoist containing class field ref into local
					final int max = size;
					while (next < max) {
						action.accept(new AbstractLong2FloatMap.BasicEntry(key[curr = next], value[next++]));
					}
				}
			};
		}
		...
		final class EntrySetSpliterator extends ObjectSpliterators.EarlyBindingSizeIndexBasedSpliterator<Long2FloatMap.Entry> implements ObjectSpliterator<Long2FloatMap.Entry> {
			...
			protected final Long2FloatMap.Entry get(int location) {
				return new AbstractLong2FloatMap.BasicEntry(key[location], value[location]);
			}
		...
		public void forEach(final Consumer<? super Long2FloatMap.Entry> action) {
			// Hoist containing class field ref into local
			for (int i = 0, max = size; i < max; ++i) {
				action.accept(new AbstractLong2FloatMap.BasicEntry(key[i], value[i]));
			}
		}
		...
		public void fastForEach(final Consumer<? super Long2FloatMap.Entry> action) {
			final BasicEntry entry = new BasicEntry();
			// Hoist containing class field ref into local
			for (int i = 0, max = size; i < max; ++i) {
				entry.key = key[i];
