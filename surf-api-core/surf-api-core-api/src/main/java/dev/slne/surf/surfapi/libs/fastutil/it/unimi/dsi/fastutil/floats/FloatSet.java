...
public interface FloatSet extends FloatCollection, Set<Float> {
	...
	default FloatSpliterator spliterator() {
		return FloatSpliterators.asSpliterator(iterator(), sizeOf(this), FloatSpliterators.SET_SPLITERATOR_CHARACTERISTICS);
	}
	...
	public static FloatSet of(float... a) {
		...
		FloatSet innerSet = a.length <= FloatSets.ARRAY_SET_CUTOFF ? new FloatArraySet(a.length) : new FloatOpenHashSet(a.length);
		for (float element : a) {
