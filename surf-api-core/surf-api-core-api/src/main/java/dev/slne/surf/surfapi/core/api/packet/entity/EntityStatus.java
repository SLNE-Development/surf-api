...
public enum EntityStatus {
    ...
    /**
     * Spawn cloud particles at the player. Sent to a player whose Bad Omen effect is removed to either start a raid or increase its difficulty.
     */
    ...
    /**
     * Resets the squid's rotation to {@code 0} radians. Occurs whenever the server calculates that the squid has rotated more than {@code 2 pi} radians.
     */
    SQUID_RESET_ROTATION(19, PacketSquid.class),

    /**
     * Causes several "happy villager" particles to appear; used when the dolphin has been fed and is locating a structure
     */
...
//    ALLAY_DUPLICATION(18, PacketAllay.class), TODO
    ...
    /**
     * Performs the sonic boom attack animation (Charge and release). Beam and sound are not included.
     */
    ...
    public void checkApplicable(PacketEntity<?> entity) {
        checkState(applicableClass.isAssignableFrom(entity.getClass()), "Entity %s is not applicable for status %s", entity.getClass().getSimpleName(), this);
    }
