 ...
 * {@code ==} instead of {@link Object#equals(Object) equals}) for its keys or values (or both),
 * which may result in breaks in contract if attempted to be used with non reference-equality
 * semantics based {@link Map}s. For example, a {@code aReferenceToIntMap.equals(aObjectToIntMap)}
 * may return different a different result then {@code aObjectToIntMap.equals(aReferenceToIntMap)},
 * in violation of {@link Object#equals equals}'s contract requiring it being symmetric.
 *
 * <p>
...
public interface Reference2FloatMap<K> extends Reference2FloatFunction<K>, Map<K, Float> {
	...
	// Defaultable methods
	@Override
	default void forEach(final java.util.function.BiConsumer<? super K, ? super Float> consumer) {
		final ObjectSet<Reference2FloatMap.Entry<K>> entrySet = reference2FloatEntrySet();
		final Consumer<Reference2FloatMap.Entry<K>> wrappingConsumer = (entry) -> consumer.accept((entry.getKey()), Float.valueOf(entry.getFloatValue()));
	 ...
	 * @since 8.0.0
	 */
	default float computeIfAbsent(final K key, final java.util.function.ToDoubleFunction<? super K> mappingFunction) {
	 ...
	 * @since 8.0.0
	 */
	default float computeIfAbsent(final K key, final Reference2FloatFunction<? super K> mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final float v = getFloat(key), drv = defaultReturnValue();
		if (v != drv || containsKey(key)) return v;
	 ...
	 * @since 8.5.0
	 */
	default float mergeFloat(final K key, final float value, final it.unimi.dsi.fastutil.floats.FloatBinaryOperator remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final float oldValue = getFloat(key), drv = defaultReturnValue();
		final float newValue = oldValue != drv || containsKey(key) ? remappingFunction.apply(oldValue, value) : value;
		put(key, newValue);
		return newValue;
	}
	 ...
	 *           this overhead.
	 */
	default float mergeFloat(final K key, final float value, final java.util.function.DoubleBinaryOperator remappingFunction) {
		return mergeFloat(key, value, remappingFunction instanceof it.unimi.dsi.fastutil.floats.FloatBinaryOperator ? (it.unimi.dsi.fastutil.floats.FloatBinaryOperator)remappingFunction : (x, y) -> it.unimi.dsi.fastutil.SafeMath.safeDoubleToFloat(remappingFunction.applyAsDouble(x, y)));
	}
