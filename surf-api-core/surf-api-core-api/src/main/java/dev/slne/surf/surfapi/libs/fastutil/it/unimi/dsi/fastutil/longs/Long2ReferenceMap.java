...
public interface Long2ReferenceMap<V> extends Long2ReferenceFunction<V>, Map<Long, V> {
	...
	default void forEach(final java.util.function.BiConsumer<? super Long, ? super V> consumer) {
		final ObjectSet<Long2ReferenceMap.Entry<V>> entrySet = long2ReferenceEntrySet();
		final Consumer<Long2ReferenceMap.Entry<V>> wrappingConsumer = (entry) -> consumer.accept(Long.valueOf(entry.getLongKey()), (entry.getValue()));
	 ...
	 * @since 8.0.0
	 */
	default V computeIfAbsent(final long key, final Long2ReferenceFunction<? extends V> mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final V v = get(key), drv = defaultReturnValue();
		if (v != drv || containsKey(key)) return v;
