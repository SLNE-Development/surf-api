...
public interface Reference2ByteMap<K> extends Reference2ByteFunction<K>, Map<K, Byte> {
	...
	// Defaultable methods
	@Override
	default void forEach(final java.util.function.BiConsumer<? super K, ? super Byte> consumer) {
		final ObjectSet<Reference2ByteMap.Entry<K>> entrySet = reference2ByteEntrySet();
		final Consumer<Reference2ByteMap.Entry<K>> wrappingConsumer = (entry) -> consumer.accept((entry.getKey()), Byte.valueOf(entry.getByteValue()));
	 ...
	 * @since 8.0.0
	 */
	default byte computeIfAbsent(final K key, final java.util.function.ToIntFunction<? super K> mappingFunction) {
	 ...
	 * @since 8.0.0
	 */
	default byte computeIfAbsent(final K key, final Reference2ByteFunction<? super K> mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final byte v = getByte(key), drv = defaultReturnValue();
		if (v != drv || containsKey(key)) return v;
	 ...
	 * @since 8.5.0
	 */
	default byte mergeByte(final K key, final byte value, final it.unimi.dsi.fastutil.bytes.ByteBinaryOperator remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final byte oldValue = getByte(key), drv = defaultReturnValue();
	 ...
	 *           this overhead.
	 */
	default byte mergeByte(final K key, final byte value, final java.util.function.IntBinaryOperator remappingFunction) {
