	...
	private final class EntrySet extends AbstractObjectSet<Long2ReferenceMap.Entry<V>> implements FastEntrySet<V> {
		...
		public ObjectIterator<Long2ReferenceMap.Entry<V>> iterator() {
			return new ObjectIterator<Long2ReferenceMap.Entry<V>>() {
				...
				@Override
				@SuppressWarnings("unchecked")
				public void forEachRemaining(final Consumer<? super Long2ReferenceMap.Entry<V>> action) {
					// Hoist containing class field ref into local
					final int max = size;
					while (next < max) {
						action.accept(new AbstractLong2ReferenceMap.BasicEntry<>(key[curr = next], (V)value[next++]));
					}
				}
			};
		}
		...
		final class EntrySetSpliterator extends ObjectSpliterators.EarlyBindingSizeIndexBasedSpliterator<Long2ReferenceMap.Entry<V>> implements ObjectSpliterator<Long2ReferenceMap.Entry<V>> {
			...
			protected final Long2ReferenceMap.Entry<V> get(int location) {
				return new AbstractLong2ReferenceMap.BasicEntry<>(key[location], (V)value[location]);
			}
		...
		@Override
		@SuppressWarnings("unchecked")
		public void forEach(final Consumer<? super Long2ReferenceMap.Entry<V>> action) {
			// Hoist containing class field ref into local
			for (int i = 0, max = size; i < max; ++i) {
				action.accept(new AbstractLong2ReferenceMap.BasicEntry<>(key[i], (V)value[i]));
			}
		}
		...
		@Override
		@SuppressWarnings("unchecked")
		public void fastForEach(final Consumer<? super Long2ReferenceMap.Entry<V>> action) {
			final BasicEntry<V> entry = new BasicEntry<>();
			// Hoist containing class field ref into local
			for (int i = 0, max = size; i < max; ++i) {
				entry.key = key[i];
