...
public interface Short2ReferenceMap<V> extends Short2ReferenceFunction<V>, Map<Short, V> {
	...
	// Defaultable methods
	@Override
	default void forEach(final java.util.function.BiConsumer<? super Short, ? super V> consumer) {
		final ObjectSet<Short2ReferenceMap.Entry<V>> entrySet = short2ReferenceEntrySet();
		final Consumer<Short2ReferenceMap.Entry<V>> wrappingConsumer = (entry) -> consumer.accept(Short.valueOf(entry.getShortKey()), (entry.getValue()));
	 ...
	 * @since 8.0.0
	 */
	default V computeIfAbsent(final short key, final Short2ReferenceFunction<? extends V> mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final V v = get(key), drv = defaultReturnValue();
		if (v != drv || containsKey(key)) return v;
