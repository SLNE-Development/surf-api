...
public interface Float2ReferenceMap<V> extends Float2ReferenceFunction<V>, Map<Float, V> {
	...
	// Defaultable methods
	@Override
	default void forEach(final java.util.function.BiConsumer<? super Float, ? super V> consumer) {
		final ObjectSet<Float2ReferenceMap.Entry<V>> entrySet = float2ReferenceEntrySet();
		final Consumer<Float2ReferenceMap.Entry<V>> wrappingConsumer = (entry) -> consumer.accept(Float.valueOf(entry.getFloatKey()), (entry.getValue()));
	 ...
	 * @since 8.0.0
	 */
	default V computeIfAbsent(final float key, final Float2ReferenceFunction<? extends V> mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final V v = get(key), drv = defaultReturnValue();
		if (v != drv || containsKey(key)) return v;
