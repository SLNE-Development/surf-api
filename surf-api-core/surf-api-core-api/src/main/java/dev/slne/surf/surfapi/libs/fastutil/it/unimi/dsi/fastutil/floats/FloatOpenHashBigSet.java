...
public class FloatOpenHashBigSet extends AbstractFloatSet implements java.io.Serializable, Cloneable, Hash, Size64 {
	...
	public boolean add(final float k) {
		...
		if ((Float.floatToIntBits(k) == 0)) {
			...
			if (!(Float.floatToIntBits(curr = key[base = (int)((h & mask) >>> BigArrays.SEGMENT_SHIFT)][displ = (int)(h & segmentMask)]) == 0)) {
				...
				while (!(Float.floatToIntBits(curr = key[base = (base + ((displ = (displ + 1) & segmentMask) == 0 ? 1 : 0)) & baseMask][displ]) == 0)) if ((Float.floatToIntBits(curr) == Float.floatToIntBits(k))) return false;
			}
			key[base][displ] = k;
		}
	...
	protected final void shiftKeys(long pos) {
		...
		for (;;) {
			...
			for (;;) {
				...
				slot = it.unimi.dsi.fastutil.HashCommon.mix((long)(it.unimi.dsi.fastutil.HashCommon.float2int(BigArrays.get(key, pos)))) & mask;
	...
	protected void rehash(final long newN) {
		...
		for (long i = realSize(); i-- != 0;) {
			...
			// The starting point.
			if (!(Float.floatToIntBits(newKey[b = (int)((h & mask) >>> BigArrays.SEGMENT_SHIFT)][d = (int)(h & newSegmentMask)]) == 0)) while (!(Float.floatToIntBits(newKey[b = (b + ((d = (d + 1) & newSegmentMask) == 0 ? 1 : 0)) & newBaseMask][d]) == 0));
			newKey[b][d] = k;
	...
	private void readObject(java.io.ObjectInputStream s) throws java.io.IOException, ClassNotFoundException {
		...
		for (long i = size; i-- != 0;) {
			...
			if ((Float.floatToIntBits(k) == 0)) containsNull = true;
				...
				h = it.unimi.dsi.fastutil.HashCommon.mix((long)(it.unimi.dsi.fastutil.HashCommon.float2int(k)));
				if (!(Float.floatToIntBits(key[base = (int)((h & mask) >>> BigArrays.SEGMENT_SHIFT)][displ = (int)(h & segmentMask)]) == 0)) while (!(Float.floatToIntBits(key[base = (base + ((displ = (displ + 1) & segmentMask) == 0 ? 1 : 0)) & baseMask][displ]) == 0));
				key[base][displ] = k;
			}
		}
		if (ASSERTS) checkTable();
