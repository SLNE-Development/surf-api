...
public class ShortOpenHashSet extends AbstractShortSet implements java.io.Serializable, Cloneable, Hash {
	 ...
	 */
	public static ShortOpenHashSet of(final short e) {
		ShortOpenHashSet result = new ShortOpenHashSet(1, DEFAULT_LOAD_FACTOR);
		result.add(e);
		return result;
	}
	 ...
	 */
	public static ShortOpenHashSet of(final short e0, final short e1) {
		ShortOpenHashSet result = new ShortOpenHashSet(2, DEFAULT_LOAD_FACTOR);
		result.add(e0);
		if (!result.add(e1)) {
	 ...
	 * @throws IllegalArgumentException if there were duplicate entries.
	 */
	public static ShortOpenHashSet of(final short e0, final short e1, final short e2) {
		ShortOpenHashSet result = new ShortOpenHashSet(3, DEFAULT_LOAD_FACTOR);
		result.add(e0);
		if (!result.add(e1)) {
			throw new IllegalArgumentException("Duplicate element: " + e1);
		}
	...
	public static ShortOpenHashSet of(final short... a) {
		ShortOpenHashSet result = new ShortOpenHashSet(a.length, DEFAULT_LOAD_FACTOR);
		for (short element : a) {
	...
	public boolean add(final short k) {
		...
		if (((k) == ((short)0))) {
			...
			if (!((curr = key[pos = (it.unimi.dsi.fastutil.HashCommon.mix((k))) & mask]) == ((short)0))) {
				...
				while (!((curr = key[pos = (pos + 1) & mask]) == ((short)0))) if (((curr) == (k))) return false;
			}
			key[pos] = k;
		}
		...
		public void forEachRemaining(final ShortConsumer action) {
			...
			while (c != 0) {
				if (--pos < 0) {
					// We are just enumerating elements from the wrapped list.
					last = Integer.MIN_VALUE;
					action.accept(wrapped.getShort(-pos - 1));
					c--;
	...
	public void forEach(final ShortConsumer action) {
		...
		final short key[] = this.key;
		for (int pos = n; pos-- != 0;) if (!((key[pos]) == ((short)0))) action.accept(key[pos]);
	}
	...
	private void readObject(java.io.ObjectInputStream s) throws java.io.IOException, ClassNotFoundException {
		...
		for (int i = size, pos; i-- != 0;) {
			...
			if (((k) == ((short)0))) {
				...
				if (!((key[pos = (it.unimi.dsi.fastutil.HashCommon.mix((k))) & mask]) == ((short)0))) while (!((key[pos = (pos + 1) & mask]) == ((short)0)));
			}
			key[pos] = k;
		}
		if (ASSERTS) checkTable();
