...
public class ShortBigArrayBigList extends AbstractShortBigList implements RandomAccess, Cloneable, java.io.Serializable {
	...
	public ShortBigArrayBigList(final ShortCollection c) {
		...
		if (c instanceof ShortBigList) {
			((ShortBigList)c).getElements(0, a, 0, size = Size64.sizeOf(c));
		} else {
	...
	public void ensureCapacity(final long capacity) {
		if (capacity <= length(a) || a == ShortBigArrays.DEFAULT_EMPTY_BIG_ARRAY) return;
	...
	public boolean addAll(long index, final ShortCollection c) {
		...
		size += n;
		assert size <= length(a);
		while (n-- != 0) BigArrays.set(a, index++, i.nextShort());
		return true;
	}
	...
	public void size(final long size) {
		...
		if (size > this.size) BigArrays.fill(a, this.size, size, ((short)0));
		this.size = size;
	}
	...
	private class SubList extends AbstractShortBigList.ShortRandomAccessSubList {
		...
		public boolean equals(Object o) {
			...
			if (o instanceof ShortBigArrayBigList.SubList) {
		...
		public int compareTo(final BigList<? extends Short> l) {
			...
			if (l instanceof ShortBigArrayBigList.SubList) {
	...
	private final class Spliterator implements ShortSpliterator {
		...
		public Spliterator() {
			this(0, ShortBigArrayBigList.this.size, false);
		}
		...
		private long getWorkingMax() {
			return hasSplit ? max : ShortBigArrayBigList.this.size;
		}
	...
	public ShortBigArrayBigList clone() {
		...
		if (getClass() == ShortBigArrayBigList.class) {
			c = new ShortBigArrayBigList(size);
			c.size = size;
		} else {
			try {
			...
			c.a = ShortBigArrays.newBigArray(size);
		}
	...
	public boolean equals(final Object o) {
		...
		if (o instanceof ShortBigArrayBigList.SubList) {
			...
			return ((ShortBigArrayBigList.SubList)o).equals(this);
		}
		return super.equals(o);
	}
	...
	public int compareTo(final BigList<? extends Short> l) {
		...
		if (l instanceof ShortBigArrayBigList.SubList) {
			...
			return -((ShortBigArrayBigList.SubList)l).compareTo(this);
		}
		return super.compareTo(l);
	}
	...
	private void writeObject(java.io.ObjectOutputStream s) throws java.io.IOException {
		...
		for (int i = 0; i < size; i++) s.writeShort(BigArrays.get(a, i));
	}

	private void readObject(java.io.ObjectInputStream s) throws java.io.IOException, ClassNotFoundException {
		s.defaultReadObject();
		a = ShortBigArrays.newBigArray(size);
		for (int i = 0; i < size; i++) BigArrays.set(a, i, s.readShort());
	}
}
