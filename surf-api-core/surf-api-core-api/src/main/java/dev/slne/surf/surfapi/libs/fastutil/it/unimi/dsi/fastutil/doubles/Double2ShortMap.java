...
public interface Double2ShortMap extends Double2ShortFunction, Map<Double, Short> {
	...
	// Defaultable methods
	@Override
	default void forEach(final java.util.function.BiConsumer<? super Double, ? super Short> consumer) {
		final ObjectSet<Double2ShortMap.Entry> entrySet = double2ShortEntrySet();
		final Consumer<Double2ShortMap.Entry> wrappingConsumer = (entry) -> consumer.accept(Double.valueOf(entry.getDoubleKey()), Short.valueOf(entry.getShortValue()));
	 ...
	 * @since 8.0.0
	 */
	default short computeIfAbsent(final double key, final Double2ShortFunction mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final short v = get(key), drv = defaultReturnValue();
		if (v != drv || containsKey(key)) return v;
	 ...
	 * @since 8.5.0
	 */
	default short mergeShort(final double key, final short value, final it.unimi.dsi.fastutil.shorts.ShortBinaryOperator remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final short oldValue = get(key), drv = defaultReturnValue();
	 ...
	 *           this overhead.
	 */
	default short mergeShort(final double key, final short value, final java.util.function.IntBinaryOperator remappingFunction) {
