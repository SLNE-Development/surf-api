	...
	private final class EntrySet extends AbstractObjectSet<Char2LongMap.Entry> implements FastEntrySet {
		...
		public ObjectIterator<Char2LongMap.Entry> iterator() {
			return new ObjectIterator<Char2LongMap.Entry>() {
				...
				public void forEachRemaining(final Consumer<? super Char2LongMap.Entry> action) {
					// Hoist containing class field ref into local
					final int max = size;
					while (next < max) {
						action.accept(new AbstractChar2LongMap.BasicEntry(key[curr = next], value[next++]));
					}
				}
			};
		}
		...
		final class EntrySetSpliterator extends ObjectSpliterators.EarlyBindingSizeIndexBasedSpliterator<Char2LongMap.Entry> implements ObjectSpliterator<Char2LongMap.Entry> {
			...
			protected final Char2LongMap.Entry get(int location) {
				return new AbstractChar2LongMap.BasicEntry(key[location], value[location]);
			}
		...
		public void forEach(final Consumer<? super Char2LongMap.Entry> action) {
			// Hoist containing class field ref into local
			for (int i = 0, max = size; i < max; ++i) {
				action.accept(new AbstractChar2LongMap.BasicEntry(key[i], value[i]));
			}
		}
		...
		public void fastForEach(final Consumer<? super Char2LongMap.Entry> action) {
			final BasicEntry entry = new BasicEntry();
			// Hoist containing class field ref into local
			for (int i = 0, max = size; i < max; ++i) {
				entry.key = key[i];
