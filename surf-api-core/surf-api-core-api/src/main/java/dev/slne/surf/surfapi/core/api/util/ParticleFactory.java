...
public interface ParticleFactory {
  ...
  static @NotNull Particle of(@NotNull ParticleType type, float scale, @NotNull TextColor start,
      @NotNull TextColor end) {
    checkNotNull(start, "Start color may not be null");
    checkNotNull(end, "End color may not be null");

    return of(type,
        new ParticleDustColorTransitionData(scale, start.red(), start.green(), start.blue(),
  ...
  @Contract("_, _, _, _ -> new")
  static @NotNull Particle of(@NotNull ParticleType type, @NotNull Vector3i startingPosition,
      @Nullable Vector3i blockPosition, int ticks) {
