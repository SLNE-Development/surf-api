...
public interface Object2FloatMap<K> extends Object2FloatFunction<K>, Map<K, Float> {
	...
	default void forEach(final java.util.function.BiConsumer<? super K, ? super Float> consumer) {
		final ObjectSet<Object2FloatMap.Entry<K>> entrySet = object2FloatEntrySet();
		final Consumer<Object2FloatMap.Entry<K>> wrappingConsumer = (entry) -> consumer.accept((entry.getKey()), Float.valueOf(entry.getFloatValue()));
	 ...
	 * @since 8.0.0
	 */
	default float computeIfAbsent(final K key, final Object2FloatFunction<? super K> mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final float v = getFloat(key), drv = defaultReturnValue();
		if (v != drv || containsKey(key)) return v;
