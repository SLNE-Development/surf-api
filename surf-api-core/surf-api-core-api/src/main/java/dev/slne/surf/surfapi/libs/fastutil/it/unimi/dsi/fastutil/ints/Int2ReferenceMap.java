...
public interface Int2ReferenceMap<V> extends Int2ReferenceFunction<V>, Map<Integer, V> {
	...
	default void forEach(final java.util.function.BiConsumer<? super Integer, ? super V> consumer) {
		final ObjectSet<Int2ReferenceMap.Entry<V>> entrySet = int2ReferenceEntrySet();
		final Consumer<Int2ReferenceMap.Entry<V>> wrappingConsumer = (entry) -> consumer.accept(Integer.valueOf(entry.getIntKey()), (entry.getValue()));
	 ...
	 * @since 8.0.0
	 */
	default V computeIfAbsent(final int key, final Int2ReferenceFunction<? extends V> mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final V v = get(key), drv = defaultReturnValue();
		if (v != drv || containsKey(key)) return v;
