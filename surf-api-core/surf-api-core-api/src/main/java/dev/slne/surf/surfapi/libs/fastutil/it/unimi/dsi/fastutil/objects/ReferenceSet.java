 ...
 * contract if attempted to be used with non reference-equality semantics based {@link Set}s. For
 * example, a {@code aReferenceSet.equals(aObjectSet)} may return different a different result then
...
public interface ReferenceSet<K> extends ReferenceCollection<K>, Set<K> {
	...
	public static <K> ReferenceSet<K> of(K... a) {
		...
		ReferenceSet<K> innerSet = a.length <= ReferenceSets.ARRAY_SET_CUTOFF ? new ReferenceArraySet<>(a.length) : new ReferenceOpenHashSet<>(a.length);
		for (K element : a) {
