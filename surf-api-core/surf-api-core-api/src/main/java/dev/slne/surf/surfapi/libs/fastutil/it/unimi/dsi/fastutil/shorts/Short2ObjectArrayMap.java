	...
	private final class EntrySet extends AbstractObjectSet<Short2ObjectMap.Entry<V>> implements FastEntrySet<V> {
		...
		public ObjectIterator<Short2ObjectMap.Entry<V>> iterator() {
			return new ObjectIterator<Short2ObjectMap.Entry<V>>() {
				...
				@Override
				@SuppressWarnings("unchecked")
				public void forEachRemaining(final Consumer<? super Short2ObjectMap.Entry<V>> action) {
					// Hoist containing class field ref into local
					final int max = size;
					while (next < max) {
						action.accept(new AbstractShort2ObjectMap.BasicEntry<>(key[curr = next], (V)value[next++]));
					}
				}
			};
		}
		...
		final class EntrySetSpliterator extends ObjectSpliterators.EarlyBindingSizeIndexBasedSpliterator<Short2ObjectMap.Entry<V>> implements ObjectSpliterator<Short2ObjectMap.Entry<V>> {
			...
			protected final Short2ObjectMap.Entry<V> get(int location) {
				return new AbstractShort2ObjectMap.BasicEntry<>(key[location], (V)value[location]);
			}
		...
		@Override
		@SuppressWarnings("unchecked")
		public void forEach(final Consumer<? super Short2ObjectMap.Entry<V>> action) {
			// Hoist containing class field ref into local
			for (int i = 0, max = size; i < max; ++i) {
				action.accept(new AbstractShort2ObjectMap.BasicEntry<>(key[i], (V)value[i]));
			}
		}
		...
		@Override
		@SuppressWarnings("unchecked")
		public void fastForEach(final Consumer<? super Short2ObjectMap.Entry<V>> action) {
			final BasicEntry<V> entry = new BasicEntry<>();
			// Hoist containing class field ref into local
			for (int i = 0, max = size; i < max; ++i) {
				entry.key = key[i];
