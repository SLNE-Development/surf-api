...
public class InvokerFactory<I> {
     ...
     *
     * @param templateClass    the template class whose .class bytecode will be used
     *                         as the hidden class template
     ...
     */
    public InvokerFactory(final Class<?> templateClass, final Class<I> invokerInterface, final MethodHandles.Lookup lookup) {
        this.invokerInterface = invokerInterface;
        this.lookup = lookup;

        try (final InputStream is = templateClass.getResourceAsStream(templateClass.getSimpleName() + ".class")) {
            Objects.requireNonNull(is, templateClass.getSimpleName() + ".class not found");
            this.templateClassBytes = is.readAllBytes();
        } catch (IOException e) {
            throw new AssertionError("Failed to load " + templateClass.getSimpleName() + ".class", e);
        }
    }
    ...
    /**
     * Creates a new invoker for the given target, method, and payload class.
     * Automatically detects whether the method is a suspend function.
     *
    ...
    public I create(final Object target, final Method method, final Class<?> payloadClass) {
        try {
            return HiddenInvokerUtil.createInvoker(lookup, templateClassBytes, invokerInterface, target, method, payloadClass);
    ...
    /**
     * Returns the pre-loaded template bytecode.
     * Needed by template classes' static initializers.
     */
