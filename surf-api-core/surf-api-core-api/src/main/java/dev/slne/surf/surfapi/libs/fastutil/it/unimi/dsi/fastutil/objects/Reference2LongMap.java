...
public interface Reference2LongMap<K> extends Reference2LongFunction<K>, Map<K, Long> {
	...
	default void forEach(final java.util.function.BiConsumer<? super K, ? super Long> consumer) {
		final ObjectSet<Reference2LongMap.Entry<K>> entrySet = reference2LongEntrySet();
		final Consumer<Reference2LongMap.Entry<K>> wrappingConsumer = (entry) -> consumer.accept((entry.getKey()), Long.valueOf(entry.getLongValue()));
	 ...
	 * @since 8.0.0
	 */
	default long computeIfAbsent(final K key, final Reference2LongFunction<? super K> mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final long v = getLong(key), drv = defaultReturnValue();
		if (v != drv || containsKey(key)) return v;
