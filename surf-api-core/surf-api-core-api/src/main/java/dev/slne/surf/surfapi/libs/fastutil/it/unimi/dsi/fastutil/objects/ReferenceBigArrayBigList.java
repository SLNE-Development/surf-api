...
public class ReferenceBigArrayBigList<K> extends AbstractReferenceBigList<K> implements RandomAccess, Cloneable, java.io.Serializable {
	...
	public ReferenceBigArrayBigList(final ReferenceCollection<? extends K> c) {
		...
		if (c instanceof ReferenceBigList) {
			((ReferenceBigList<? extends K>)c).getElements(0, a, 0, size = Size64.sizeOf(c));
		} else {
	 ...
	 */
	public ReferenceBigArrayBigList(final Collection<? extends K> c) {
		this(Size64.sizeOf(c));
		if (c instanceof ReferenceBigList) {
			((ReferenceBigList<? extends K>)c).getElements(0, a, 0, size = Size64.sizeOf(c));
		} else {
	 ...
	 */
	@SafeVarargs
	public static <K> ReferenceBigArrayBigList<K> of(final K... init) {
	...
	private static final Collector<Object, ?, ReferenceBigArrayBigList<Object>> TO_LIST_COLLECTOR = Collector.of(ReferenceBigArrayBigList::new, ReferenceBigArrayBigList::add, ReferenceBigArrayBigList::combine);
	...
	public static <K> Collector<K, ?, ReferenceBigArrayBigList<K>> toBigListWithExpectedSize(long expectedSize) {
		return Collector.of(() -> new ReferenceBigArrayBigList<K>(expectedSize), ReferenceBigArrayBigList::add, ReferenceBigArrayBigList::combine);
	}
	...
	public boolean addAll(long index, final Collection<? extends K> c) {
		...
		if (c instanceof ReferenceBigList) {
			return addAll(index, (ReferenceBigList<? extends K>)c);
		}
		ensureIndex(index);
		int n = c.size();
		if (n == 0) return false;
	...
	@Override
	public boolean addAll(final long index, final ReferenceBigList<? extends K> list) {
		ensureIndex(index);
	...
	@Override
	public boolean addAll(final long index, final ReferenceList<? extends K> list) {
		ensureIndex(index);
		int n = list.size();
		if (n == 0) return false;
		grow(size + n);
	...
	private class SubList extends AbstractReferenceBigList.ReferenceRandomAccessSubList<K> {
		...
		public boolean equals(Object o) {
			...
			if (o instanceof ReferenceBigArrayBigList.SubList) {
				@SuppressWarnings("unchecked")
				ReferenceBigArrayBigList<K>.SubList other = (ReferenceBigArrayBigList<K>.SubList)o;
	...
	private final class Spliterator implements ObjectSpliterator<K> {
		...
		public Spliterator() {
			this(0, ReferenceBigArrayBigList.this.size, false);
		}
		...
		private long getWorkingMax() {
			return hasSplit ? max : ReferenceBigArrayBigList.this.size;
		}
	...
	public ReferenceBigArrayBigList<K> clone() {
		...
		if (getClass() == ReferenceBigArrayBigList.class) {
			c = new ReferenceBigArrayBigList<>(size);
			c.size = size;
		} else {
			try {
	...
	public boolean equals(final Object o) {
		...
		if (o instanceof ReferenceBigArrayBigList) {
			...
			return equals((ReferenceBigArrayBigList<K>)o);
		}
		if (o instanceof ReferenceBigArrayBigList.SubList) {
			// Safe cast because we are only going to take elements from other list, never give them
			// Sublist has an optimized sub-array based comparison, reuse that.
			return ((ReferenceBigArrayBigList<K>.SubList)o).equals(this);
		}
		return super.equals(o);
	}
