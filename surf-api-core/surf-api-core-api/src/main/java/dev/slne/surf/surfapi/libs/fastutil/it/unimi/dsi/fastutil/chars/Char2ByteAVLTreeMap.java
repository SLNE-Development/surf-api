	...
	*/
	...
	public ObjectSortedSet<Char2ByteMap.Entry> char2ByteEntrySet() {
		if (entries == null) entries = new AbstractObjectSortedSet<Char2ByteMap.Entry>() {
			final Comparator<? super Char2ByteMap.Entry> comparator = (Char2ByteAVLTreeMap.this.actualComparator == null ? (Comparator<Char2ByteMap.Entry>)(x, y) -> (Character.compare((x.getCharKey()), (y.getCharKey()))) : (Comparator<Char2ByteMap.Entry>)(x, y) -> Char2ByteAVLTreeMap.this.actualComparator.compare(x.getCharKey(), y.getCharKey()));
	...
	private final class Submap extends AbstractChar2ByteSortedMap implements java.io.Serializable {
		...
		final boolean in(final char k) {
			return (bottom || Char2ByteAVLTreeMap.this.compare(k, from) >= 0) && (top || Char2ByteAVLTreeMap.this.compare(k, to) < 0);
		}
