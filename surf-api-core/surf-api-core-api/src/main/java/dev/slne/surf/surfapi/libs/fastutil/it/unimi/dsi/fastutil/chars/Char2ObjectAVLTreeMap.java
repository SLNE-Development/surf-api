	...
	*/
	...
	public ObjectSortedSet<Char2ObjectMap.Entry<V>> char2ObjectEntrySet() {
		if (entries == null) entries = new AbstractObjectSortedSet<Char2ObjectMap.Entry<V>>() {
			final Comparator<? super Char2ObjectMap.Entry<V>> comparator = (Char2ObjectAVLTreeMap.this.actualComparator == null ? (Comparator<Char2ObjectMap.Entry<V>>)(x, y) -> (Character.compare((x.getCharKey()), (y.getCharKey()))) : (Comparator<Char2ObjectMap.Entry<V>>)(x, y) -> Char2ObjectAVLTreeMap.this.actualComparator.compare(x.getCharKey(), y.getCharKey()));
	...
	private final class Submap extends AbstractChar2ObjectSortedMap<V> implements java.io.Serializable {
		...
		final boolean in(final char k) {
			return (bottom || Char2ObjectAVLTreeMap.this.compare(k, from) >= 0) && (top || Char2ObjectAVLTreeMap.this.compare(k, to) < 0);
		}
