	...
	private final class EntrySet extends AbstractObjectSet<Float2IntMap.Entry> implements FastEntrySet {
		...
		public ObjectIterator<Float2IntMap.Entry> iterator() {
			return new ObjectIterator<Float2IntMap.Entry>() {
				...
				public void forEachRemaining(final Consumer<? super Float2IntMap.Entry> action) {
					// Hoist containing class field ref into local
					final int max = size;
					while (next < max) {
						action.accept(new AbstractFloat2IntMap.BasicEntry(key[curr = next], value[next++]));
					}
				}
			};
		}
		...
		final class EntrySetSpliterator extends ObjectSpliterators.EarlyBindingSizeIndexBasedSpliterator<Float2IntMap.Entry> implements ObjectSpliterator<Float2IntMap.Entry> {
			...
			protected final Float2IntMap.Entry get(int location) {
				return new AbstractFloat2IntMap.BasicEntry(key[location], value[location]);
			}
		...
		public void forEach(final Consumer<? super Float2IntMap.Entry> action) {
			// Hoist containing class field ref into local
			for (int i = 0, max = size; i < max; ++i) {
				action.accept(new AbstractFloat2IntMap.BasicEntry(key[i], value[i]));
			}
		}
		...
		public void fastForEach(final Consumer<? super Float2IntMap.Entry> action) {
			final BasicEntry entry = new BasicEntry();
			// Hoist containing class field ref into local
			for (int i = 0, max = size; i < max; ++i) {
				entry.key = key[i];
