...
public class ModernSerializers {
  ...
  public static class LinkedListSerializer extends AbstractListChildSerializer<LinkedList<?>> {
    ...
    static final TypeToken<LinkedList<?>> TYPE = new TypeToken<>() {
    };
    ...
    protected Type elementType(Type containerType) throws SerializationException {
      if (!(containerType instanceof ParameterizedType parameterizedType)) {
        throw new SerializationException(containerType,
    ...
    @Override
    protected LinkedList<?> createNew(int length, Type elementType) {
    ...
    protected void deserializeSingle(int index, LinkedList<?> collection,
      ...
      //noinspection unchecked, rawtypes - we know the type is correct
