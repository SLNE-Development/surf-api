...
public interface Short2FloatMap extends Short2FloatFunction, Map<Short, Float> {
	...
	// Defaultable methods
	@Override
	default void forEach(final java.util.function.BiConsumer<? super Short, ? super Float> consumer) {
		final ObjectSet<Short2FloatMap.Entry> entrySet = short2FloatEntrySet();
		final Consumer<Short2FloatMap.Entry> wrappingConsumer = (entry) -> consumer.accept(Short.valueOf(entry.getShortKey()), Float.valueOf(entry.getFloatValue()));
	 ...
	 * @since 8.0.0
	 */
	default float computeIfAbsent(final short key, final Short2FloatFunction mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final float v = get(key), drv = defaultReturnValue();
		if (v != drv || containsKey(key)) return v;
	 ...
	 * @since 8.5.0
	 */
	default float mergeFloat(final short key, final float value, final it.unimi.dsi.fastutil.floats.FloatBinaryOperator remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final float oldValue = get(key), drv = defaultReturnValue();
	 ...
	 *           this overhead.
	 */
	default float mergeFloat(final short key, final float value, final java.util.function.DoubleBinaryOperator remappingFunction) {
