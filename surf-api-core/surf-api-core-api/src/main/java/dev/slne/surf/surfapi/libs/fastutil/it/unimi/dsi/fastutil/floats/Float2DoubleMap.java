...
public interface Float2DoubleMap extends Float2DoubleFunction, Map<Float, Double> {
	...
	// Defaultable methods
	@Override
	default void forEach(final java.util.function.BiConsumer<? super Float, ? super Double> consumer) {
		final ObjectSet<Float2DoubleMap.Entry> entrySet = float2DoubleEntrySet();
		final Consumer<Float2DoubleMap.Entry> wrappingConsumer = (entry) -> consumer.accept(Float.valueOf(entry.getFloatKey()), Double.valueOf(entry.getDoubleValue()));
	 ...
	 * @since 8.0.0
	 */
	default double computeIfAbsent(final float key, final Float2DoubleFunction mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final double v = get(key), drv = defaultReturnValue();
		if (v != drv || containsKey(key)) return v;
	 ...
	 * @since 8.5.0
	 */
	default double mergeDouble(final float key, final double value, final java.util.function.DoubleBinaryOperator remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final double oldValue = get(key), drv = defaultReturnValue();
	 ...
	 * @since 8.5.1
	 */
	default double mergeDouble(final float key, final double value, final it.unimi.dsi.fastutil.doubles.DoubleBinaryOperator remappingFunction) {
