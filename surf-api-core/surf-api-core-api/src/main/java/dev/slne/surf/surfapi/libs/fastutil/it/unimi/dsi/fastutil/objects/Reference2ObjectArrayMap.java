...
public class Reference2ObjectArrayMap<K, V> extends AbstractReference2ObjectMap<K, V> implements java.io.Serializable, Cloneable {
	...
	private final class EntrySet extends AbstractObjectSet<Reference2ObjectMap.Entry<K, V>> implements FastEntrySet<K, V> {
		...
		public ObjectIterator<Reference2ObjectMap.Entry<K, V>> iterator() {
			return new ObjectIterator<Reference2ObjectMap.Entry<K, V>>() {
				...
				public void forEachRemaining(final Consumer<? super Reference2ObjectMap.Entry<K, V>> action) {
					...
					final int max = size;
					while (next < max) {
						action.accept(new AbstractReference2ObjectMap.BasicEntry<>((K)key[curr = next], (V)value[next++]));
					}
				}
			};
		}
		...
		final class EntrySetSpliterator extends ObjectSpliterators.EarlyBindingSizeIndexBasedSpliterator<Reference2ObjectMap.Entry<K, V>> implements ObjectSpliterator<Reference2ObjectMap.Entry<K, V>> {
			...
			protected final Reference2ObjectMap.Entry<K, V> get(int location) {
				return new AbstractReference2ObjectMap.BasicEntry<>((K)key[location], (V)value[location]);
			}
		...
		public void forEach(final Consumer<? super Reference2ObjectMap.Entry<K, V>> action) {
			...
			for (int i = 0, max = size; i < max; ++i) {
				action.accept(new AbstractReference2ObjectMap.BasicEntry<>((K)key[i], (V)value[i]));
			}
		}
