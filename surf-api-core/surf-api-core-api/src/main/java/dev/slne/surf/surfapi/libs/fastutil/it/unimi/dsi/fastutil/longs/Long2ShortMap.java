...
public interface Long2ShortMap extends Long2ShortFunction, Map<Long, Short> {
	...
	// Defaultable methods
	@Override
	default void forEach(final java.util.function.BiConsumer<? super Long, ? super Short> consumer) {
		final ObjectSet<Long2ShortMap.Entry> entrySet = long2ShortEntrySet();
		final Consumer<Long2ShortMap.Entry> wrappingConsumer = (entry) -> consumer.accept(Long.valueOf(entry.getLongKey()), Short.valueOf(entry.getShortValue()));
	 ...
	 * @since 8.0.0
	 */
	default short computeIfAbsent(final long key, final Long2ShortFunction mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final short v = get(key), drv = defaultReturnValue();
		if (v != drv || containsKey(key)) return v;
	 ...
	 * @since 8.5.0
	 */
	default short mergeShort(final long key, final short value, final it.unimi.dsi.fastutil.shorts.ShortBinaryOperator remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final short oldValue = get(key), drv = defaultReturnValue();
	 ...
	 *           this overhead.
	 */
	default short mergeShort(final long key, final short value, final java.util.function.IntBinaryOperator remappingFunction) {
