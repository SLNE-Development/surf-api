package dev.slne.surf.api.core.server.impl.component
...
class SurfComponentApiImpl : SurfComponentApi {
    ...
    override suspend fun disable(owner: Any) {
        ...
        val components = ComponentService.get().awaitLoadedComponents(owner)
        for (index in components.lastIndex downTo 0) {
            components[index].disable()
        }
    }
    ...
    override suspend fun components(owner: Any): List<SurfComponent> {
        return ComponentService.get().getOrLoadComponents(owner)
    }

    override fun componentsLoaded(owner: Any): List<SurfComponent> {
