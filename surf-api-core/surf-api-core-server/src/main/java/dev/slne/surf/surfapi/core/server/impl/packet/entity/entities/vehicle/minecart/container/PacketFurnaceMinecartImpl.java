...
public final class PacketFurnaceMinecartImpl extends
    PacketAbstractMinecartContainerImpl<PacketFurnaceMinecart> implements PacketFurnaceMinecart {

  private static final WrappedBlockState FURNACE_BLOCK_STATE = BlockStateFactory.builder(
      StateTypes.FURNACE).facing(BlockFace.NORTH).build();
  private static final WrappedBlockState FURNACE_BLOCK_STATE_LIT = BlockStateFactory.builder(
      FURNACE_BLOCK_STATE).lit(true).build();
  ...
  public void hasFuel(boolean hasFuel, boolean updateDisplayedBlockState) {
    ...
    if (updateDisplayedBlockState) {
      set(DISPLAY_BLOCK_DATA_INDEX,
          hasFuel ? FURNACE_BLOCK_STATE_LIT.getGlobalId() : FURNACE_BLOCK_STATE.getGlobalId());
    }
