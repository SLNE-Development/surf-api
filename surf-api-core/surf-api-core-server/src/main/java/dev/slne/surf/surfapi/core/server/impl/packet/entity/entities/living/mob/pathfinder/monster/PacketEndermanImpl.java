...
public final class PacketEndermanImpl extends PacketMonsterImpl<PacketEnderman> implements PacketEnderman {
    ...
    public Optional<WrappedBlockState> carriedBlock() {
        return Optional.<Integer>ofNullable(get(CARRIED_BLOCK_INDEX, null)).map(WrappedBlockState::getByGlobalId);
    }

    @Override
    public void carriedBlock(@Nullable WrappedBlockState blockData) {
        setOptInt(CARRIED_BLOCK_INDEX, Optional.ofNullable(blockData).map(WrappedBlockState::getGlobalId));
        afterSet();
    }
