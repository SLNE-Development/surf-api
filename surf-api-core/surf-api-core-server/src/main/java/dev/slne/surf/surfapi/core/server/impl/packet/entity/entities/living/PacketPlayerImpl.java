...
public final class PacketPlayerImpl extends PacketLivingEntityImpl<PacketPlayer> implements
    PacketPlayer { // TODO: testing bc I think it`s broken
  ...
  public PacketPlayerImpl(UUID uuid) {
    ...
    this.npcProfile = new UserProfile(uuid, LegacyComponentSerializer.legacySection()
        .serialize(displayName().orElse(Component.text("Player"))));
  }
  ...
  @Override
  public @NotNull CompletableFuture<Void> skinProperties(@NotNull UUID playerUuid,
      boolean respawn) {
  ...
  public void showOnTabList(boolean showOnTabList) {
    ...
    if (spawned) {
      sendPacketToAllViewers(version -> infoPacket(version, Action.UPDATE_LISTED,
          listed ? WrapperPlayServerPlayerInfo.Action.ADD_PLAYER
  ...
  public void ping(@NotNull Ping ping) {
    ...
    if (spawned) {
      sendPacketToAllViewers(version -> infoPacket(version, Action.UPDATE_LATENCY,
  ...
  public void gameMode(@NotNull GameMode gameMode) {
    ...
    if (spawned) {
      sendPacketToAllViewers(version -> infoPacket(version, Action.UPDATE_GAME_MODE,
  ...
  public void displayName(@Nullable Component displayName) {
    ...
    npcProfile.setName(PlainTextComponentSerializer.plainText()
        .serialize(displayName != null ? displayName : Component.text("Player")));
    if (spawned) {
      sendPacketToAllViewers(version -> infoPacket(version, Action.UPDATE_DISPLAY_NAME,
  ...
  private PacketWrapper<?> spawnPlayerPacket(ClientVersion version) {
    ...
    if (version.isNewerThanOrEquals(ClientVersion.V_1_20_2)) {
      ...
      return new WrapperPlayServerSpawnPlayer(entityId(), uuid(), location,
          this.entityData(version));
    }
  }

  private PacketWrapper<?> infoPacket(ClientVersion version, Action modernAction,
