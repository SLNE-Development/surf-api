...
public abstract class PacketLivingEntityImpl<Impl extends PacketLivingEntity<Impl>> extends
    PacketEntityImpl<Impl> implements PacketLivingEntity<Impl> {
  ...
  public Optional<org.spongepowered.math.vector.Vector3i> bedLocation() {
    return get(BED_LOCATION_INDEX, Optional.<Vector3i>empty()).map(
        PacketEntityCommon::fromPacketEvents);
  }

  @Override
  public void bedLocation(@Nullable org.spongepowered.math.vector.Vector3i bedLocation) {
    setOptBlockPos(BED_LOCATION_INDEX,
        Optional.ofNullable(bedLocation).map(PacketEntityCommon::toPacketEvents));
    afterSet();
  }
